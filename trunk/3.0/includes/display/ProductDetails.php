	...
	*/
	...
	private function SetPricingDetails()
	{
		...
		if(GetConfig('isIntelisis'))
		{
			...
			if($saleprice != '') {
				$product['prodcalculatedprice'] = applyPyC($product); //Precio a mostrar en la lista, regresado por PyC
	...
	public function SetBrandDetails()
	{
		...
		// Show config of ShowProductBrandImage
		$ProdBrandId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT prodbrandid FROM [|PREFIX|]products WHERE productid = "'.$GLOBALS['ProductId'].'"');
		$BrandImage = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT brandimagefile FROM [|PREFIX|]brands WHERE brandid = "'.$ProdBrandId.'"');
		if(GetConfig('ShowProductBrandImage')) {
			if ($BrandImage !== '') {
				$GLOBALS['ShowBrandImage'] = GetConfig('ShopPath') . '/' . GetConfig('ImageDirectory') . '/' . $BrandImage;
			}
			else {
				$GLOBALS['ShowBrandImage'] = $GLOBALS['IMG_PATH'].'/BrandDefault.gif';
			}
		}
		else {
			$GLOBALS['ProductShowBrandDisplay'] = 'none';
		}
