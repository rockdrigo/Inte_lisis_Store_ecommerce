<?php
	...
	class TEMPLATE
	{
		...
		*/
		...
		public function ParseTemplateLangFile()
		{
			$backLangFile = dirname(__FILE__) . "/../../templates/".$this->getActiveTemplateName()."/overrides.ini";
			if(file_exists($backLangFile)) {
				ParseLangFile($backLangFile);
			}
		}
		...
		*/
		public function Parse($prefix, $text, $replace)
		{
			...
			if(GetConfig('DebugMode') && $prefix == 'Panel.'){
				$lastendtime = microtime_float();
				$sesstoken = $GLOBALS['ISC_CLASS_SESSION']->GetToken();
			}
			...
			foreach ($matches[0] as $key => $k) {
				...
				if(GetConfig('DebugMode') && $prefix == 'Panel.'){
					...
					$paneltime = number_format($endtime - $lastendtime, 4);
					$lastendtime = $endtime;
					logAddNotice($sesstoken.'|Panel: "'.$k.'"|('.$endtime.'-'.$lastendtime.')|'.$paneltime);
				}
			}
