...
class ISC_ADMIN_LAYOUT extends ISC_ADMIN_BASE
{
	...
	{
		$control = mysql_connect('localhost', 'adminselect', 'g2madmselpwd') or die('Connect error: ' . mysql_error());
		mysql_selectdb('tv_control');

		$clave = substr(GetConfig('tablePrefix'), 0, -1);
		
		$q_asigned = 'SELECT Usuario FROM asignacion_templates WHERE Clave = "'.$clave.'"';
		$r_asigned = mysql_query($q_asigned) or die('Invalid query: ' . mysql_error());
		
		while ($row = mysql_fetch_array($r_asigned)) {
			$templatedir = ISC_BASE_PATH.'/templates/'.$templateName;
			$userdir = '/home/'.$row['Usuario'].'/tiendas/'.$clave.'/'.$templateName;
			...
			$c_perms = 'sudo chown -R apache:apache '.$templatedir. ' && sudo chmod -R a+w '.$templatedir;
			$r_perms = system($c_perms);
		}
	}
