...
<script type="text/javascript" charset="utf-8">
	...
	function AddTotalRange(node)
	{
		...
		var oldName = $('.PayPalMultiAccountsDiv:last input:first').attr('name');
		...
		$(newNode).find('select').each(function() {
			parts = $(this).attr('name').split(/[_|\[|\]]/);
			$(this).attr('name', parts[0] + '_' + parts[1] + '[' + parts[2] + '_' + newNum + ']');
			$(this).attr('id', parts[2] + '_' + newNum);
			$(this).val('');
	...
	{
		$('.PayPalMultiAccountsDiv:first a.remove').hide();
		$('.PayPalMultiAccountsDiv:gt(0) a.remove').show();
	}
