...
class ISC_INTELISIS_ECOMMERCE_WEBUSUARIO extends ISC_INTELISIS_ECOMMERCE {
	...
	private function createCustomer() {
		...
		if($StoreOriginFieldId = getCustomFieldId(FORMFIELDS_FORM_ACCOUNT, GetLang('StoreOriginLabel'))) {
			$_GET['FormField'][FORMFIELDS_FORM_ACCOUNT][$StoreOriginFieldId] = $this->getData('Sucursal', '');
		}
		...
		if ($customer->AccountWithEmailAlreadyExists($customerData['custconemail'])) {
			logAdd(LOG_SEVERITY_ERROR, 'Interfaz con Intelisis intento crear un Cliente con Mail ya existente: '.$customerData['custconemail']);
			return false;
		}
		// Else is the provided phone number valid?
		else if (!$customer->ValidatePhoneNumber($customerData['custconphone'])) {
			logAdd(LOG_SEVERITY_ERROR, 'Interfaz con Intelisis intento crear un Cliente con Telefono no valido: '.$customerData['custconphone']);
			return false;
		}
		else {
			...
			if (isId($customerId)) {
				$insert = array('GUID' => $this->getData('GUID'),
						'customerid'=>$customerId, 
						'Cliente'=>$this->getData('Cliente'),
						'IDWebUsuario' => $this->getData('ID'),
						'Sucursal' => $this->getData('Sucursal'),
				);
				if(!($rtn_insert = $GLOBALS['ISC_CLASS_DB']->InsertQuery('intelisis_customers', $insert, true)))
				{
	...
	private function updateCustomer() {
			...
			$query = sprintf("select * from [|PREFIX|]customers where custconemail = '".$this->getData('eMail')."' and customerid != '".$customer_id."'");
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
			if($row){
				logAdd(LOG_SEVERITY_ERROR, 'Interfaz con Intelisis intento crear un Cliente con Mail ya existente: '.$this->getData('eMail'));
				return false;
			}
			
			$customerDetails = $this->GetAccountDetails();
			...
			if($StoreOriginFieldId = getCustomFieldId(FORMFIELDS_FORM_ACCOUNT, GetLang('StoreOriginLabel'))) {
				$_POST[___FORM_DEFAULT_NAME___][FORMFIELDS_FORM_ACCOUNT][$StoreOriginFieldId] = $this->getData('Sucursal', '');
			}
			...
			{
				$update = array('Cliente' => $this->getData('Cliente'),
						'customerid' => $customer_id,
						'Sucursal' => $this->getData('Sucursal'),
				);
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery('intelisis_customers', $update, 'GUID = "'.$this->getData('GUID').'"');
	...
	private function deleteCustomer() {
		...
		if ($customerId) {
			...
			logAdd(LOG_SEVERITY_WARNING, 'No se pudo encontrar el customer id de "'.$this->getAttribute('IDUsuario').'". Archivo: '.$this->getXMLfilename());
			return true;
		}
	}
}