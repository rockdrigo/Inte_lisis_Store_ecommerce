...
class ISC_PREQUOTE_ADDRESS
{
	...
	/**
	 * @var ISC_PREQUOTE Quote instance this address belongs to.
	 */
	protected $quote;

	/**
	 * @var string One of the ISC_PREQUOTE_ADDRESS::TYPE_* constants.
	 */
	protected $type;
	...
	public function setGUID($GUID = ''){
		if($GUID != ''){
			$this->GUID = $GUID;
			logAddNotice('Le puse GUID "'.$this->GUID.'" existente a address de tipo "'.$this->getType().'"');
			return;
		}

		if($this->getCustomerAddressId() == 0){
			$this->GUID = gen_uuid();
			logAddNotice('Generando nuevo GUID "'.$this->GUID.'" para address de tipo "'.$this->getType().'"');			
		}
		else {
			$GUID = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT GUID FROM [|PREFIX|]intelisis_shipping_addresses WHERE shipid = "'.$this->getCustomerAddressId().'"', 'GUID');
			$this->GUID = $GUID;
			logAddNotice('Puse GUID "'.$this->GUID.'" para address de tipo "'.$this->getType().'"');
		}
	}
	...
	*
	* @param string $hash
	* @return ISC_PREQUOTE_ITEM or false if not found on this address
	*/
	public function getItemByHash($hash)
	{
		foreach ($this->getItems() as /** @var ISC_PREQUOTE_ITEM */$item) {
	...
	*/
	...
	public function getTaxSummary()
	{
		...
		if ($this instanceof ISC_PREQUOTE_ADDRESS_SHIPPING) {
	...
	* @return ISC_PREQUOTE
	*/
	public function moveAllItemsToAddress(ISC_PREQUOTE_ADDRESS $address)
	{
		$items = array();
		foreach ($this->getItems() as /** @var ISC_PREQUOTE_ITEM */$item) {
