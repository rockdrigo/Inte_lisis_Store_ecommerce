<?php
	...
	class ISC_PRODUCT
	{
		...
		public function _SetProductData($productid=0)
		{
			...
			if($this->_prodallowpurchases == 1 && GetConfig('UsePreCart')) {
				if(!productHasStock($this->_prodsku)){
					FlashMessage('No hay Existencias de este producto en los Almacenes. No lo podrá agregal al carrito.', MSG_INFO);
				}
				if(checkForStoreOriginNecessary($this->_prodid)){
					FlashMessage('En la Sucursal Favorita del Cliente no hay Existencia. Por favor elija otra Sucursal', MSG_INFO);
				}
			}
		}
		...
		*/
		...
		public function GetProductFields($productId)
		{
			...
			if(checkForStoreOriginNecessary($productId)){
				$fields[] = array(
						"id"			=> 0,
						"name"			=> 'Sucursal',
						"type"			=> 'select',
						"fileType"		=> '',
						"fileSize"		=> '',
						"selectOptions"	=> getStoreOriginOptions($productId),
						"required"		=> 1,
						"layer"			=> '',
				);
			}
