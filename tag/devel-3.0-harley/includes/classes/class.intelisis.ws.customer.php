...
class ISC_INTELISIS_WS_CUSTOMER extends ISC_INTELISIS_WS {
	...
	public function __construct($customerId, $customerData, $action) {
		...
		$xml = '<?xml version="1.0" encoding="windows-1252"?><Intelisis Sistema="Intelisis" Contenido="Solicitud" Referencia="eCommerce.Intelisis.WebUsuario" SubReferencia="'.$this->GUID.'" Compania="'.$this->empresa.'"><Solicitud Empresa ="'.$this->empresa.'" Sucursal="'.getCustomerStoreOrigin($this->customerId).'" Estatus="'.$Estatus.'" ><Usuario GUID="'.$this->GUID.'" Nombre="'.$this->customerData['custconfirstname'].'" Apellidos="'.$this->customerData['custconlastname'].'" eMail= "'.$this->customerData['custconemail'].'" Contrasena="'.$this->customerData['custpassword'].'" Telefono="'.$this->customerData['custconphone'].'" RFC="'.$RFC.'" Compania="'.$this->customerData['custconcompany'].'">';
