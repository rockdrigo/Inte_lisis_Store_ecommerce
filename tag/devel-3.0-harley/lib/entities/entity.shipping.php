...
class ISC_ENTITY_SHIPPING extends ISC_ENTITY_BASE
{
	...
	protected function addPosthook($shipId, $savedata, $rawInput)
	{
		//ToDo: Evitar hacer esto cuando se agrega la entidad que viene por la respuesta de IS
		if(!isFromCron('pendingjobs') && GetConfig('isIntelisis') && (GetConfig('syncIWSurl') != '' || GetConfig('syncDropboxActive') == 1))
		{
			if(!isset($rawInput['GUID']) || $rawInput['GUID'] == '') $rawInput['GUID'] = gen_uuid();
			$IWS = new ISC_INTELISIS_WS_CUSTOMER_ADDRESS($shipId, $rawInput, 'add');
	...
	protected function editPosthook($shipId, &$savedata, $rawInput)
	{
		if(!isFromCron('pendingjobs') && GetConfig('isIntelisis') && (GetConfig('syncIWSurl') != '' || GetConfig('syncDropboxActive') == 1))
		{
			$IWS = new ISC_INTELISIS_WS_CUSTOMER_ADDRESS($shipId, $rawInput, 'edit');
			if(!$IWS->prepareRequest()) return false;
		}
	}
