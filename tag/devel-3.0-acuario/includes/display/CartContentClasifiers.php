<?php
class ISC_CARTCONTENTCLASIFIERS_PANEL extends PANEL
{
	protected $quote;
	...
	public function generateCartContent()
	{
		...
		$GLOBALS['SNIPPETS']['CartItemGroups'] = "";

		$deliveryDates = array();
		$itemsClasifiers = $this->quote->getItemsClasifiers();
		foreach($itemsClasifiers as $clasifier => $items) {
			...
			$GLOBALS['CartGroupItems'] = '';
			if(trim($clasifier) != ''){
				$GLOBALS['CartItemsClasifier'] = sprintf(GetLang('CartItemsOfClasifier'), $clasifier);
			}
			else {
				$GLOBALS['CartItemsClasifier'] = GetLang('CartItemsNoClasifier');
			}
			...
			foreach($items as $item) {
				...
				$GLOBALS['CartGroupItems'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("CartItem");
			...
			$GLOBALS['SNIPPETS']['CartItemGroups'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("CartItemGroupClasifiers");
		}
