...
BEGIN
  ...
  IF pGetListaPreciosCliente = 1 THEN
  	/* NES - Todavia no tengo soporte de esto
    SELECT pCliente = Cliente,
	   EnviarA
      into
      vEnviarA
      FROM %%PREFIX%%intelisis_Venta 
     WHERE ID = pVentaID;
     */
    SELECT 
	--  NES - No tengo soporte a nivel EnviarA
	-- Agregar columna a intelisis_shipping_addresses y joinear ahi
	  IFNULL(/*IFNULL(cea.ListaPreciosEsp,*/ c.ListaPreciosEsp/*)*/, '(Precio Lista)')
      INTO
	  pListaPreciosEsp
	  FROM %%PREFIX%%intelisis_Cte c
	 /*INNER JOIN %%PREFIX%%intelisis_CteEnviarA cea 
        ON c.Cliente = cea.Cliente AND cea.ID = vEnviarA*/; 
  END IF;
...
CREATE PROCEDURE %%PREFIX%%spPoliticaPreciosCalc
			(
				in pFechaEmision				datetime,
				in pAgente						varchar(10),
				in pMoneda						varchar(10), 
				...
				in pCantidad					decimal(30,10),
				in pUnidadVenta				varchar(50),
				inout pPrecio					decimal(30,10),
				inout pDescuento				decimal(30,10),
				inout pPolitica				longtext,
				inout pDescuentoMonto			decimal(30,10)
			)
BEGIN
  ...
  SELECT 
   ...
   SELECT 
    ...
    /* NES - Siempre sacamos el UltimoCosto
   -- Obtiene el tipo de costeo
   SELECT 
		TipoCosteo
	 INTO
		vTipoCosteo
     FROM %%PREFIX%%intelisis_EmpresaCfg ec 
    WHERE ec.Empresa = pEmpresa;
   */
...
DELIMITER //
DROP TRIGGER IF EXISTS `%%PREFIX%%sincronizacion_tgbi`//
...
DROP PROCEDURE IF EXISTS %%PREFIX%%spPCGetOriginal;
...
/* NES pPrecioOriginal NUNCA va a ser uno, spPCGet es llamado siempre con 0
  ...
  IF pPrecio IS NULL THEN
    IF SUBSTRING(pLista, 1, 1) = '(' THEN
      IF     pLista = '(Precio Lista)'  THEN SELECT a.MonedaPrecio, a.PrecioLista  INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 2)'      THEN SELECT a.MonedaPrecio, a.Precio2      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 3)'      THEN SELECT a.MonedaPrecio, a.Precio3      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 4)'      THEN SELECT a.MonedaPrecio, a.Precio4      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 5)'      THEN SELECT a.MonedaPrecio, a.Precio5      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 6)'      THEN SELECT a.MonedaPrecio, a.Precio6      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 7)'      THEN SELECT a.MonedaPrecio, a.Precio7      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 8)'      THEN SELECT a.MonedaPrecio, a.Precio8      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 9)'      THEN SELECT a.MonedaPrecio, a.Precio9      INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio 10)'     THEN SELECT a.MonedaPrecio, a.Precio10     INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Precio Minimo)' THEN SELECT a.MonedaPrecio, a.PrecioMinimo INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
      ELSEIF pLista = '(Incentivo)'     THEN SELECT a.MonedaPrecio, a.Incentivo    INTO vArtMoneda, pPrecio FROM %%PREFIX%%intelisis_Art a WHERE a.Articulo = Articulo; 
...
ELSE
	...
	THEN
		SELECT
			vArtMoneda 	= MonedaCosto, 
		   vUltimo    	= UltimoCosto,
		   vUltimoSinGastos = UltimoCostoSinGastos,
		   vEstandar  	= Art.CostoEstandar,
		   vReposicion	= Art.CostoReposicion,
		   vPromedio  	= CostoPromedio,
		   vPrecio	= IFNULL(vPrecio, PrecioLista),
		   vMargen	= Margen,
		   vImpuesto1   = Art.Impuesto1
		INTO
			vArtMoneda, 
		   vUltimo,
		   vUltimoSinGastos,
	...
	ELSE
		SELECT vArtMoneda 	= MonedaCosto, 
		   ...
		   vPrecio	= IFNULL(vPrecio, PrecioLista),
		   vMargen      = Margen,
		   vImpuesto1   = IFNULL(Art.Impuesto1, 0),
		   vImpuesto2   = IFNULL(Art.Impuesto2, 0),
		   vImpuesto3   = IFNULL(Art.Impuesto3, 0)
		INTO
			vArtMoneda,
		   vUltimo,
		   vUltimoSinGastos,
