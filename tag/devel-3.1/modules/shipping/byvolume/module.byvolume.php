<?php
	...
	*/
	class SHIPPING_BYVOLUME extends ISC_SHIPPING
	{
		...
		*/
		public function __construct()
		{
			...
			$GLOBALS['VolumeMeasurement'] = GetConfig('VolumeMeasurement');

			// Setup the required variables for the flat rate per item shipping module
			parent::__construct();
			$this->_name = GetLang('ByVolumeName');
			$this->_image = "";
			$this->_description = GetLang('ByVolumeDesc');
			$this->_help = GetLang('ByVolumeHelp');
			$this->_height = 0;
		...
		*/
		...
		public function getpropertiessheet($tab_id)
		{
			...
			$output = $this->ParseTemplate('volume_range_header', true);
		...
		public function BuildForm()
		{
			...
			if (empty($this->rules['cost'])) {
				...
				$GLOBALS['LOWER_VAL'] = '';
				$GLOBALS['UPPER_VAL'] = '';
				return $this->ParseTemplate('volume_range_row', true);
			}
			...
			foreach ($this->rules['cost'] as $id => $cost) {
				...
				if($this->rules['lower'][$id] !== '') {
					$GLOBALS['LOWER_VAL'] = FormatVolume($this->rules['lower'][$id], false);
				}

				$GLOBALS['UPPER_VAL'] = '';
				if($this->rules['upper'][$id] !== '') {
					$GLOBALS['UPPER_VAL'] = FormatVolume($this->rules['upper'][$id], false);
				}
				$output .= $this->ParseTemplate('volume_range_row', true);
			}
		...
		{
			return $this->ParseTemplate('volume_range_javascript', true);
		}

		private function LoadVolumeRanges()
		{
			$query = "SELECT variablename, variableval
			FROM [|PREFIX|]shipping_vars
			WHERE methodid='".$this->methodId."' AND modulename='shipping_byvolume'
			AND (
		...
		private function GetCost()
		{
			...
			if (($this->rules['lower'][0] === '' && $this->rules['upper'][0] !== '') && $this->_volume < $this->rules['upper'][0]) {
				return $this->rules['cost'][0];
			} elseif (($this->rules['upper'][$last_rule] === '' && $this->rules['lower'][0] !== '') && $this->_volume >= $this->rules['lower'][$last_rule]) {
				return $this->rules['cost'][$last_rule];
			}

			foreach (array_keys($this->rules['cost']) as $key) {
				if (isset($this->rules['lower'][$key]) && $this->_volume >= $this->rules['lower'][$key] && isset($this->rules['upper'][$key]) && $this->_volume < $this->rules['upper'][$key]) {
