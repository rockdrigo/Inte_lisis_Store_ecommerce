...
class ACCOUNTING_QUICKBOOKS_SERVICE_PRODUCTADD extends ACCOUNTING_QUICKBOOKS_SERVICE_BASE
{
		...
		if (is_array($this->spool["children"]) && !empty($this->spool["children"])) {
			...
			switch (isc_strtolower($lastKid["service"])) {
				case "query":
					 ...
					 */
					if ($lastKid["errNo"] > 0) {
						throw new QBException("Caught a QBJD error when adding a product record", $lastKid);
					}
					...
					return $this->execChildService("product", "edit", $this->spool["nodeData"]);
					break;
				...
				case "edit":
					 ...
					 */
					if ($lastKid["errNo"] > 0) {
						throw new QBException("Caught a QBJD error when editing a product record (from productadd)", $lastKid);
					}
	...
	{
		return $this->execChildService("product", "query", $this->spool["nodeData"]);
	}
}