	...
	*/
	...
	private function SetShippingCost()
	{
		...
		if ($this->productClass->GetFixedShippingCost() != 0) {
			...
			$GLOBALS['ShippingPrice'] = sprintf("%s %s", CurrencyConvertFormatPrice($this->productClass->GetFixedShippingCost()/*,$defaultCurrency,null,true*/), GetLang('FixedShippingCost'));
		}
	...
	private function SetPricingDetails()
	{
		...
		if(GetConfig('isIntelisis'))
		{
			...
			$return =  applyPyC($product);
			$saleprice = $return['Precio'];
			$toCurrency = $return['Moneda'];
			
			$discountPrice = $saleprice - ($saleprice * ($return['Descuento'] / 100));
			
			if($saleprice != '') {
				$product['prodcalculatedprice'] = $discountPrice; //Precio a mostrar en la lista, regresado por PyC
		...
		// If prices are hidden, then we don't need to go any further
			...
			/*if($toCurrency != $defaultCurrency['currencyname']){*/
			$GLOBALS['ProductDetailPrice'] = formatProductDetailsPrice($product, $options, $toCurrency);
			/*}else{
