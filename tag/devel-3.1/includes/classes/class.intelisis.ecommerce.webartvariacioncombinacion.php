...
class ISC_INTELISIS_ECOMMERCE_WEBARTVARIACIONCOMBINACION extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function createCombination() {
		...
		if($this->getData('Precio') == NULL){
			$convPrecio = '0';
		}else{
			if($this->getCurrencyId() != $this->GetDefaultCurrency()){
		...
		if($this->getData('NombreImagen') != ''){
			...
			$imgpath = GetConfig('syncDropboxDir').$Clave.DIRECTORY_SEPARATOR.GetConfig('syncDropboxImagesDir').DIRECTORY_SEPARATOR.$this->getAttribute('IDArticulo').DIRECTORY_SEPARATOR.$this->getData('NombreImagen');
