...
class ISC_INTELISIS_ECOMMERCE_WEBART extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function createProduct() {
		...
		if($currencyId != GetDefaultCurrency()){
			...
			$otherCurrency = GetCurrencyById($currencyId);
			...
			if($otherCurrency['currencyexchangerate'] != '0'){
				$convPrecio = $this->getData('Precio')/$otherCurrency['currencyexchangerate'];
			}else{
				$convPrecio = $this->getData('Precio');
			}
			
			$convPrecio = $this->getData('Precio')/$currencyEx;
			$convPrecioFormat = FormatPrice($convPrecio,false,false,true,$currencyId);
			if($this->getData('PrecioCImpuesto')!=''){
				$convPrecioImp = $this->getData('PrecioCImpuesto')/$currencyEx;
				$convPrecioImpFormat = FormatPrice($convPrecioImp,false,false,true,$currencyEx);
			}else{
				$convPrecioImpFormat = $convPrecioFormat;
			}
		}else{
			$convPrecio = $this->getData('Precio');
			$convPrecioFormat = DefaultPriceFormat($convPrecio);
			if($this->getData('PrecioCImpuesto')!=''){
				$convPrecioImp = $this->getData('PrecioCImpuesto');
				$convPrecioImpFormat = DefaultPriceFormat($convPrecioImp);
			}else{
				$convPrecioImpFormat = $convPrecioFormat;
			}
		}
		printe($convPrecioImpFormat);
		$productData = array(
				...
				"prodcalculatedprice" => $convPrecioImpFormat,
				...
				"prodcurrentinv" => '',
				"prodlowinv" => '',
				"prodoptionsrequired" => $this->getData('OpcionesReq'),
				...
				"prodvariationid" => $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT variationid FROM [|PREFIX|]intelisis_variations WHERE VariacionID = "'.$this->getData('VariacionID').'"', 'variationid'),
				//NES - REQ10156
	...
	private function updateProduct() {
		...
		if($this->getCurrencyId() != $defaultCurrency['currencyid']){
