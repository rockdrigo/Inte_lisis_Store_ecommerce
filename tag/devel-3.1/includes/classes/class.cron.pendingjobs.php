...
class ISC_CRON_PENDINGJOBS {
	...
	public function setNumberToProcess($i) {
		$this->numberToProcess = (is_int($i) && $i > 0) ? $i : 100;
	}
	
	public function processDropboxDir(){
		$files = array();
		$store_dir_handle = opendir($this->dropbox_dir.$this->Clave);
		if (!$store_dir_handle) {
			return false;
		}
		...
		$j = 0;
		while ($this->numberToProcess != $j && false !== ($xml_file = readdir($store_dir_handle))) {
		...
		if(GetConfig('isIntelisis')){
			...
			$chmod = 'find /var/www/vhosts/'.$this->Clave.'/htdocs/product_images/ -user sincro -exec chmod a+w {} +';
			$out = array();
	...
	private function processFile($file) {
		...
		$xml_contents = file_get_contents($this->dropbox_dir.$this->Clave.'/'.$file);
		try {
		...
		foreach($xml_errors as $file => $errors) {
			logAdd(LOG_SEVERITY_ERROR, 'Error al procesar XML "'.$file.'"', implode('<br/>', $errors));
			rename($this->dropbox_dir.$this->Clave.'/'.$file, $this->dropbox_dir.$this->Clave.'/Errores/'.$file);
			printe(" [ERROR]");
			return;
		}
		...
		if(isset($xml_dom)) {
			...
			if(GetConfig('isIntelisis')){
				...
				else
				{
					...
					$class->setXMLfilename($this->dropbox_dir.$this->Clave.'/'.$file);
					if($class->ProcessData())
					{
						rename($this->dropbox_dir.$this->Clave.'/'.$file, $this->dropbox_dir.$this->Clave.'/procesados/'.$file);
						print("[Y]");
					}
					else 
					{
