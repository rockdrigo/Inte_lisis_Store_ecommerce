...
class ISC_QUOTE_ITEM
{
	...
	*/
	...
	public function applyConfiguration($configuration)
	{
		...
		foreach ($configurableFields as $fieldId => $field) {
			...
			//JIB REC14659 Verifica los campos configurables cuando son de un articulo tipo juego
			$intelisisConfigFields = array();
			$intelisisConfigFields = $this->getIntelisisConfigFields($fieldId);
			
			if ($intelisisConfigFields['EsComponenteJuego'] == '1'){
				
				if($configuration[$fieldId] != (int)$configuration[$fieldId]){
					$arreglo = array(
						'campo' => $field['fieldname']
					);
					throw new ISC_QUOTE_EXCEPTION(getLang('GameProductInteger', $arreglo));
				}
				
				if((int)$configuration[$fieldId] < (int)$intelisisConfigFields['CantidadJuego']){
					$arreglo = array(
						'campo' => $field['fieldname'],
						'cantidad' => $intelisisConfigFields['CantidadJuego'],
					);
					throw new ISC_QUOTE_EXCEPTION(getLang('GameProductMinQuantity', $arreglo));
				}
			}
			//REC14659
	...
	//JIB REC14659 Trae la linea de la tabla de intelisis_configurable_fields dependiendo del productfieldid
	public function getIntelisisConfigFields($fieldId)
	{
		$intelisisConfigFields = array();
		$query = "
			SELECT *
			FROM [|PREFIX|]intelisis_configurable_fields
			WHERE productfieldid='".$fieldId."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$intelisisConfigFields = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		
		return $intelisisConfigFields;
	}
	//REC14659
	...
	*/
	...
	public function getTotal($incTax = null)
	{
		...
		return getClass('ISC_TAX')->round(($price * $this->getQuantity()) - $this->getDiscountAmount(), null);
	}
	...
	*/
	...
	public function afterAddedtoCart() {
		...
		if(GetConfig('isIntelisis')){
			$quantity = $this->getQuantity();
			$return = applyPyC($this->getProductData(), $quantity);
			$newprice = $return['Precio'];
			$discount = $return['Descuento'];
			$discountAmount = ($newprice * ($discount / 100)) * $quantity;

			if($newprice != '')
			{
				$this->setBasePrice($newprice, true);
				$this->addDiscount('Descuento', $discountAmount);
			}
		}
