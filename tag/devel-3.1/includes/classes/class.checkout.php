	...
	*/
	private function SaveShippingProvider()
	{
		...
		if(GetConfig('isIntelisis') && GetConfig('syncIWSurl') != ''){
			...
			if(!$IWS_preorder->prepareRequest());
		}
	...
	*/
	...
	public function BuildOrderConfirmation()
	{
		...
		foreach ($items as $item) {
			...
			$defaultCurrency = GetDefaultCurrency();
			$GLOBALS['ProductPrice'] = currencyConvertFormatPrice($price, $GLOBALS['CurrentCurrency'],null,true);
		...
		foreach($totalRows as $id => $totalRow) {
			...
			$value = currencyConvertFormatPrice($totalRow['value'],$defaultCurrency['currencyid'],null,true);
		...
		$GLOBALS['GrandTotal'] = currencyConvertFormatPrice($grandTotal, $defaultCurrency['currencyid'],null,true);
		if($grandTotal == 0) {
			$GLOBALS['HidePaymentOptions'] = "none";
