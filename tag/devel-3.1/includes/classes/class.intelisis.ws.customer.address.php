...
class ISC_INTELISIS_WS_CUSTOMER_ADDRESS extends ISC_INTELISIS_WS {
	...
	public function __construct($shippingid, $action) {
		parent::__construct();
		...
		$xml = '<?xml version="1.0" encoding="windows-1252"?><Intelisis Sistema="Intelisis" Contenido="Solicitud" Referencia="eCommerce.Intelisis.WebUsuarioEnviarA" SubReferencia="'.$this->GUID.'" Version="1.0"><Solicitud Empresa="'.$this->empresa.'" Sucursal="'.GetConfig('syncIWSintelisissucursal').'" Estatus="'.$Estatus.'" ><Direccion GUID="'.$this->GUID.'" ID="'.$IDEnviarA.'" GUIDUsuario="'.$GUIDUsuario.'" UsuarioID="'.$IDWebUsuario.'" Cliente="'.$Cliente.'" Nombre="'.$ShippingAddress['shipfirstname'].'" Apellido="'.$ShippingAddress['shiplastname'].'" Direccion1="'.$ShippingAddress['shipaddress1'].'" NumeroExterior="'.$NoExt.'" Direccion2="'.$ShippingAddress['shipaddress2'].'" Colonia="'.$Colonia.'" Delegacion="'.$Delegacion.'" Ciudad="'.$ShippingAddress['shipcity'].'" Pais="'.$ShippingAddress['shipcountry'].'" Estado="'.$ShippingAddress['shipstate'].'" CP="'.$ShippingAddress['shipzip'].'" Telefonos="'.$ShippingAddress['shipphone'].'" Vivienda="'.$Vivienda.'" /></Solicitud></Intelisis>';
	...
	public function handleIWSResult() {
		...
		if($this->action == 'add') $GLOBALS['ISC_CLASS_DB']->InsertQuery('intelisis_shipping_addresses', array('GUID' => $this->GUID, 'shipid' => $this->shippingid));
		if($this->getOk() == 0)
		{
