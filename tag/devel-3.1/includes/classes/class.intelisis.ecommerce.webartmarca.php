...
class ISC_INTELISIS_ECOMMERCE_WEBARTMARCA extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function createBrand() {
		...
		if(!in_array(isc_strtolower($brand), $current_brands) && trim($brand) != "") {
			...
			logAdd(LOG_SEVERITY_ERROR, 'La marca "'.$brand.'" ya existe');
			return false;
		}
	...
	private function updateProduct() {
		...
		if(!$brandId)
		{
			logAdd(LOG_SEVERITY_WARNING, 'No se pudo encontrar el brandid de la Marca ID "'.$this->getAttribute('IDMarca').'" para editar');
			return false;
		}
