...
class ISC_INTELISIS_ECOMMERCE_WEBARTVIDEO extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function createVideo() {
		...
		$query = sprintf("select * from [|PREFIX|]product_videos where video_id = '". $this->getAttribute('IDVideo')."'");
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	...
	private function updateVideo() {
		...
		/*$query = sprintf("select * from [|PREFIX|]product_videos where video_id = '".$videoDetails."'");
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$array = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		if(!$array){
			return $this->createVideo();
