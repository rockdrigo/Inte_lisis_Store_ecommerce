 ...
 */
function formatProductCatalogPrice($product, array $options = array(), $toCurrency = NULL)
{
	...
	if($displayFormat != TAX_PRICES_DISPLAY_BOTH) {
		...
		return formatProductPrice($product, $product['prodcalculatedprice'], $options, $toCurrency);
	}

	$options['displayInclusive'] = TAX_PRICES_DISPLAY_INCLUSIVE;
	$priceIncTax = formatProductPrice($product, $product['prodcalculatedprice'], $options, $toCurrency);

	$options['displayInclusive'] = TAX_PRICES_DISPLAY_EXCLUSIVE;
	$priceExTax = formatProductPrice($product, $product['prodcalculatedprice'], $options, $toCurrency);
 ...
 * @return string Generated HTML to display the price for the product.
 */
function formatProductDetailsPrice($product, array $options = array(), $toCurrency)
{
 ...
 */
function formatProductPrice($product, $price, array $options = array(), $toCurrency=null)
{
	...
	if($options['currencyConvert'] ) {
		$actualPrice = ConvertPriceToCurrency($actualPrice,$toCurrency);
	}
	...
	if($product['prodretailprice'] > 0 && $options['strikeRetail'] && $product['prodretailprice'] > $actualPrice ) {
		...
		//$rrp = convertPriceToCurrency($rrp, $toCurrency);

		$output .= '<strike class="RetailPriceValue">'.FormatPrice($rrp,false,true,false,$toCurrency,true).'</strike> ';
	}

	if($product['prodsaleprice'] > 0 && $product['prodsaleprice'] < $product['prodprice']) {
		$output .= '<span class="SalePrice">'.formatPrice($actualPrice,false,true,false,$toCurrency,true).'</span>';
	}
	else {
		$output .= formatPrice($actualPrice, false, true, false, $toCurrency, true);
	}
