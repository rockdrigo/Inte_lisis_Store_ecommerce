...
function CompletePendingOrder($pendingOrderToken, $status, $sendInvoice=true)
{
	...
	foreach($orderData['orders'] as $order) {
		...
		//NES: Se cambio este UpdateQuery a entity->edit para que haga validacion y si hay condiciones de interfaz, haga los llamados necesarios
		// ToDo: Probar que no falle la funcion de la entidad, especialmente las validaciones extras en parseInput, etc
		//$GLOBALS['ISC_CLASS_DB']->UpdateQuery("orders", $updatedOrder, "orderid='".$order['orderid']."'");
		$entity = new ISC_ENTITY_ORDER();
		$orderEdit = $entity->edit($updatedOrder, $order['orderid']);
	}
