...
function SetupCurrency()
{
	...
	if(CustomerIsSignedIn()){
		...
		$ClienteId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT Cliente FROM [|PREFIX|]intelisis_customers WHERE customerid = "'.$customerId.'"', 'Cliente');
		$customerCurrency = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT DefMoneda FROM [|PREFIX|]intelisis_cte WHERE Cliente = "'.$ClienteId.'"', 'DefMoneda');
		$defaultCurrency = GetDefaultCurrency();
		if($customerCurrency != $defaultCurrency['currencyname'] && $customerCurrency != NULL){
			$currencyId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT currencyid FROM [|PREFIX|]currencies WHERE currencyname = "'.$customerCurrency.'"', 'currencyid');
			if(!$currencyId){
				$currencyId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT currencyid FROM [|PREFIX|]currencies WHERE currencyisdefault = "1"', 'currencyid');
			}
			$_SESSION['CURRENCY'] = $currencyId;
		}
		if($customerCurrency == $defaultCurrency['currencyname']){
			$currencyId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT currencyid FROM [|PREFIX|]currencies WHERE currencyisdefault = "1"', 'currencyid');
			$_SESSION['CURRENCY'] = $currencyId;
		}
	}
...
function ConvertPriceToCurrency($price, $toCurrency=null, $forcedExchangeRate=null, $fromCurrency=null)
{
	...
	if($toCurrency == null) {
		if(!isset($GLOBALS['CurrentCurrency'])) {
			...
			//cuando no mando currency entra aqui en lugar de antes porque esta set $GLOBALS['CurrentCurrency']
	...
	if(!is_null($forcedExchangeRate)) {
		$toCurrency['currencyexchangerate'] = $forcedExchangeRate;
	}
