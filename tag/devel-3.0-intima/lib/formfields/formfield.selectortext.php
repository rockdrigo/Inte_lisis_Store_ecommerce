...
class ISC_FORMFIELD_SELECTORTEXT extends ISC_FORMFIELD_BASE
{
	...
	public function loadForFrontend()
	{
		...
		foreach ($options as $val) {
			...
			$selected = '';
			if (strtoupper($val) == strtoupper((string)$this->value)) {
		...
		$extraHTML = '';
		if(isset($this->extraInfo['extraHTML'])) {
			$extraHTML .= $this->extraInfo['extraHTML'];
		}
		 ...
		 */
		if (empty($options)) {
			$formField = '<input type="text" name="' . $fieldName . '" id="' . $fieldId . '" class="FormField JSHidden ' . $fieldClass . '" style="display: none; ' . $fieldStyle . '" value="' . $defaultValue . '" ' . $additionalTextOptions . ' '.$extraHTML.' />';
		 ...
		 */
			...
			$formField = '<select name="' . $fieldName . '" id="' . $fieldId . '" class="FormField JSHidden ' . $fieldClass . '" style="display: none; ' . $fieldStyle . '" '.$extraHTML.'>';
