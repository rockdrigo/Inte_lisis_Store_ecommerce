...
class MySQLDb extends Db
{
	...
	*/
	function Connect($hostname=null, $username=null, $password=null, $databasename=null)
	{
		...
		$connection_result = @mysql_connect($hostname, $username, $password, true, 131072);
	...
	function Query($query='')
	{
		...
		if(GetConfig('DebugMode')){
			...
			if(isset($trace[1]) && isset($trace[1]['file'])){
				$query = '/* '.$trace[1]['file'].' */'.PHP_EOL . $query;
				
				unset($trace[0]);
				$fulltrace = '';
				foreach($trace as $item){
					if(isset($item['file'])) $fulltrace .= $item['file'];
					if(isset($item['line'])) $fulltrace .= ' ('.$item['line'].')';
					$fulltrace .= PHP_EOL;
				}
				$query .= PHP_EOL.'/* '.$fulltrace.' */';
			}
		}
	...
	*/
	function CountResult($resource=null)
	{
		...
		if(!$resource) {
			$this->SetError('Resource is false!');
			return false;
		}
	...
	*/
	 ...
	 * TableHasField
	 *
	 * Checks if $tablename has a field named $fieldname. $table must be called without prefix
	 * 
	 * @param String The name of the table to check the field for. Do not include the prefix
	 * @param String The name of the field to check if exists in the table
	 *
	 * @return Boolean Returns true if the field exists, false if it doesnt
	 */
	function TableHasField($tablename = '', $fieldname = '')
	{
		if(trim($tablename) == '' || trim($fieldname) == ''){
			$this->SetError('Tablename or Fieldname are empty');
			return false;
		}
		
		if(trim($this->_databasename) == ''){
			$this->SetError('Databasename is empty');
			return false;
		}
		...
		$query = "SELECT * 
					...
					AND TABLE_NAME = '[|PREFIX|]".$tablename."' 
