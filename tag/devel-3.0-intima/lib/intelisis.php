...
function applyPyC($product, $quantity = 1, $combination = '') {
	...
	"'.$Sucursal.'", "'.$listaPrecios.'", "'.$Cliente['Cliente'].'", NULL, '.$GetSucursalCliente.', '.$SucursalCliente.');';
...
function GetIntelisisStockSQL(){
	if(GetConfig('UseStoreOriginForStock')){
		return ', IFNULL(SUM(ii.Existencia), 0) AS Existencia';
	}
	else {
		return '';
	}
...
function getStoreOriginOptions() {
	static $stores = array();
	...
	if(empty($stores)){
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
			$stores[$row['Sucursal']] = $row['Nombre'];
		}
	}
...
function getDefaultShippingMethodOptions($zoneId = '') {
	...
	if(empty($methods)){
		...
		$availableMethods = GetShippingMethodsByZone($zoneId);

		foreach($availableMethods as $method){
			$stores[$method['methodid']] = htmlentities($method['methodname']);
		}
	}
