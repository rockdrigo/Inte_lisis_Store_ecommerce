...
function MountTemplates(){
	// do the ajax request
	document.getElementById('TemplateVersionCheck').innerHTML = '<em>{% lang 'MountingTemplates' %}...</em>';
	jQuery.ajax({ url: 'remote.php', type: 'POST', dataType: 'xml',
		data: {'w': 'mountTemplates'},
		success: function(xml) {
			if($('status', xml).text() == 1){
				document.getElementById('TemplateVersionCheck').innerHTML = '<em>{% lang 'MountedTemplates' %}...</em>';
			}
		}
	});
}
		...
		<table class="Panel">
			...
			<tr>
				...
				<td align="left" valign="top"  style="padding:5px 5px 5px 10px;">
					...
					<input type="button" value="{% lang 'BrowseTemplateFiles' %}" class="SmallButton" class="Button" onclick="LaunchEditor();">
					<input type="Button" class="SmallButton" onclick="CheckTemplateVersion();" value="{% lang 'CheckNewVersion' %}"  id="TemplateVersionCheckButton">
					<input type="Button" class="SmallButton" onclick="DownloadNewVersion();" value="{% lang 'DownloadNewVersion' %}"  id="DownloadNewVersionButton" style="display:none; font-weight: bold;">
					<input type="button" value="{% lang 'MountTemplates' %}" class="SmallButton" class="Button" onclick="MountTemplates();"><br /><br />
