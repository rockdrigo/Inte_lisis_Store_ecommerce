...
class ISC_ADMIN_EXPORT extends ISC_ADMIN_BASE
{
	...
	public function HandleToDo($do)
	{
		...
		if(isc_strtolower($do) == 'exportwebart'){
			$this->exportWebArt();
		}
	...
	private function exportWebArt(){
		//create query to select as data from your table
		$select = "SELECT * FROM [|PREFIX|]ArticulosWeb";
		
		//run mysql query and then count number of fields
		$export = $GLOBALS['ISC_CLASS_DB']->Query($select);
		...
		$data = '';
		$first = true;
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($export)) {
			$line = '';
			
			if(array_key_exists('prodcatids', $row)){
				$catids = explode(',', $row['prodcatids']);
				$row['Categorias'] = implode(',', getCatName($catids));
				
				$lowcat = min($catids);
				$row['Categoria1'] = implode(',', getCatName($lowcat));
			}
		...
		header( "Content-disposition: filename=ArticulosWeb-".date("Y-m-d").".csv");
		print "$header\n$data";
		exit;
	}
