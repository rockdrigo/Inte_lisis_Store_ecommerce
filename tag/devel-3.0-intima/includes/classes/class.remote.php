	...
	class ISC_REMOTE extends ISC_XML_PARSER
	{
		...
		public function HandleToDo()
		{
			...
			switch  ($what) {
				...
				case 'getfieldsactivatedbyname': {
					$this->GetFieldsActivatedByName();
					break;
				}
				case 'getstoreoriginoptions': {
					$this->GetStoreOriginOptions();
					break;
				}
			}
		}
		...
		private function GetStoreOriginOptions(){
			...
			if(trim($stores) != '') {
				...
				$options .= '<option value="No hay Sucursales">No hay Sucursales</option>'.PHP_EOL;
			}

			echo isc_json_encode(array('HTML' => $options));
			exit;
		}


		public function GetFieldsActivatedByName(){
			if(!isset($_REQUEST['productId']) || !isset($_REQUEST['fieldId']) || !isset($_REQUEST['fieldValue'])){
				echo  isc_json_encode(array('error' => 'Faltan datos'));
			}
			$fieldIdReq = substr($_REQUEST['fieldId'], 12);
				
			$IDCampo = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT IDCampo FROM [|PREFIX|]intelisis_configurable_fields WHERE productfieldid = "'.$fieldIdReq.'"');
			if(!$IDCampo){
				echo  isc_json_encode(array('fieldId' => ''));
				exit;
			}
			$OpcionIntelisis = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT OpcionIntelisis
					FROM [|PREFIX|]intelisis_configurable_fields_detail
					WHERE ID = "'.$IDCampo.'" AND Valor ="'.$_REQUEST['fieldValue'].'"');
			if(!$OpcionIntelisis){
				echo  isc_json_encode(array('fieldId' => ''));
				exit;
			}			
			$fieldId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT pcf.productfieldid
					FROM [|PREFIX|]product_configurable_fields pcf
					JOIN [|PREFIX|]intelisis_configurable_fields icf on (pcf.productfieldid=icf.productfieldid)
					WHERE pcf.fieldprodid = "'.$_REQUEST['productId'].'" AND icf.OpcionIntelisis = "'.$OpcionIntelisis.'"');
			echo isc_json_encode(array('fieldId' => $fieldId));
			exit;
		}
		...
		public function ProductRefreshStock($productCode = false, $outputJSON = false) {
			...
			$consultarIWS = false;
			$query = sprintf("SELECT * FROM [|PREFIX|]intelisis_inv WHERE SKU = '".$productCode."' ORDER BY Fecha ASC LIMIT 0,1");
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$arraytime = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
			if(($arraytime['Fecha']+GetConfig('syncIWSintelisisstocktime')) < time()){
				$consultarIWS = true;
			}
			
			if($consultarIWS && GetConfig('syncIWSurl') != ''){
				...
				if(!isset($out['error'])) {
					foreach ($out as $index => $stock) {
						...
						/*
						 * NES - Movi todo este codigo a una funcion para reutilizarlo
						 */
		...
		private function checkDefaultShippingMethod(){
			$quote = getCustomerQuote();
			...
			foreach (array_keys($fields) as $fieldId) {
				if (isc_strtolower($fields[$fieldId]->record['formfieldlabel']) == isc_strtolower(GetLang('DefaultShippingMethodLabel'))) {
					...
					foreach($availableMethods as $quoteId => $method){
						if(isc_strtolower(isc_html_escape($method['description'])) ==  isc_strtolower($fields[$fieldId]->GetValue())){
							$shippingAddressId = $shippingAddress->getId();
							$_POST['selectedShippingMethod'][$shippingAddressId] = $quoteId;
						}
					}
				}
			}
			...
			if($success){
				...
				exit;
					...
					$response = array(
						...
						'changeStep' => 'BillingAddress',
						'resetSteps' => 1,
						'errorMessage' => 'Ocurrio un error al asignar el metodo de envío',
					);
					echo isc_json_encode($response);
					exit;
			}
		...
		public function saveExpressCheckoutShippingAddress()
		{
			...
			if(!empty($_POST['save_shipping_address'])) {
				getCustomerQuote()->getShippingAddress()->setSaveAddress(true);
			}
		...
		public function saveExpressCheckoutShippingProvider()
		{
			...
			if($shippingAddress->getShippingCost() > 0){
				$completedSteps[0]['message'] .=
		...
		*/
		public function GetVariationOptions($customerGroupId = null)
		{
			...
			if(count($optionIdsArray) == $product['vnumoptions']) {
				...
				// NES - Aplico Politicas y Listas de Precios si cambio, ya que pudo haber cambiado
				if(GetConfig('isIntelisis')){
					$newprice = applyPyC($productClass->GetProduct(), 1, $combination['combinationid']);
						
					if($newprice != '')
					{
						$combinationDetails['price'] = formatPrice($newprice);
					}
				}
