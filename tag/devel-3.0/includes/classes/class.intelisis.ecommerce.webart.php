...
class ISC_INTELISIS_ECOMMERCE_WEBART extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function getGiftwrapIds(){
		$IDs = $this->getData('EnvolturaIDS', '');
		...
		$return = array();
		$wrappings = array();
		$result = $GLOBALS['ISC_CLASS_DB']->Query('SELECT * FROM [|PREFIX|]intelisis_gift_wrapping');
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
			$wrappings[$row['IDEnvoltura']] = $row['wrapid'];
		}
		foreach($IDs as $id){
			if(array_key_exists($id, $wrappings))
				$return[] = $wrappings[$id];
		}
	...
	private function createTaxClass(){
		if($this->getData('Impuestos', 0) == 0){
			return 0;
		}
		
		$taxRateId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT id FROM [|PREFIX|]tax_rates WHERE name = "Impuesto Normal"', 'id');
		if(!$taxRateId){
			$insertTaxRates = array(
					'tax_zone_id' => 1,
					'name' => 'Impuesto Normal',
					'priority' => 0,
					'enabled' => 1,
					'default_rate' => 0,
			);
			$GLOBALS['ISC_CLASS_DB']->InsertQuery('tax_rates', $insertTaxRates);
			$taxRateId = $GLOBALS['ISC_CLASS_DB']->LastId();
			logAdd(LOG_SEVERITY_SUCCESS, 'Se creo el rate de impuestos "Impuesto Normal" ya que no existia.<br>Archivo: '.$this->getXMLfilename());
		}
		
		$taxRate = ($this->getData('Impuestos', 0)/$this->getData('Precio', 0))*100;
		$taxClassId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT id FROM [|PREFIX|]tax_classes WHERE name = "Articulos con Impuesto #'.$taxRate.'#"', 'id');
		
		if(!$taxClassId){
			$GLOBALS['ISC_CLASS_DB']->InsertQuery('tax_classes', array('name' => 'Articulos con Impuesto #'.$taxRate.'#'));
			$taxClassId = $GLOBALS['ISC_CLASS_DB']->LastId();
			logAdd(LOG_SEVERITY_SUCCESS, 'Se creo la clase de impuestos "Articulos con Impuesto #'.$taxRate.'#" para el articulo "'.$this->getData('Nombre').'".<br>Archivo: '.$this->getXMLfilename());
		}
	
		$taxRateClassId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT id FROM [|PREFIX|]tax_rate_class_rates WHERE tax_rate_id = "'.$taxRateId.'" AND tax_class_id = "'.$taxClassId.'"', 'id');
		if(!$taxRateClassId){
			$insertTaxRateClass = array(
					'tax_rate_id' => $taxRateId,
					'tax_class_id' => $taxClassId,
					'rate' => $taxRate,
			);
			$GLOBALS['ISC_CLASS_DB']->InsertQuery('tax_rate_class_rates', $insertTaxRateClass);
			logAdd(LOG_SEVERITY_SUCCESS, 'Se relaciono la clase de Impuestos "Articulos con Impuesto #'.$taxRate.'#" con el rate Impuesto Normal.<br>Archivo: '.$this->getXMLfilename());
		}
		return $taxClassId;
	}
	...
	private function createProduct() {
		...
		$giftwrappings = $this->getGiftwrapIds();
		...
		$productData = array(
				...
				"prodretailprice" => DefaultPriceFormat($this->getData('Precio')),
				"prodsaleprice" => DefaultPriceFormat($this->getData('PrecioOferta')),
				"prodcalculatedprice" => DefaultPriceFormat($this->getData('PrecioOferta')!='' ? $this->getData('PrecioOferta') : $this->getData('Precio')),
				...
				"prodminqty" => '',
				"prodmaxqty" => '',
				'tax_class_id' => $this->createTaxClass(),
	...
	private function updateProduct() {
		...
		{
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery('intelisis_products', array(
				'ArticuloID'=>$this->getAttribute('IDArticulo'),
				'Articulo' => $this->getData('Articulo'),
				'productid'=>$productId,
				'Situacion'=>$this->getData('Situacion')
