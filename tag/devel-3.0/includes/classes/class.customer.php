...
class ISC_CUSTOMER
{
	public function HandlePage()
	{
		...
		switch ($action)
		{
			...
			default: {
				$this->ShowLoginPage($msg, $msgstatus);
			}
		}
	}
						...
						if ($row['custnotes'] != "PASSWORD") {
							...
							if ($row['custnotes'] == "INTENTO1" || $row['custnotes'] == "INTENTO2") {
								if ($row['custnotes'] == "INTENTO2") {
									...
									if ($entity->edit($UpdateNotes, $customerid))
	...
	private function LoginCustomer($ClientRecord, $silent=false)
	{
		...
		if (isset($ClientRecord['custnotes']) && $ClientRecord['custnotes'] == "INACTIVO") {
			//$this->ShowLoginPage("ActivateAccountActivate", 1);
			header(sprintf("Location: %s/login.php?msg=ActivateAccountActivate", $GLOBALS['ShopPathNormal']));
			die();
		}
	...
	*/
	private function ActivateAccount() {
		...
		if (isset($_GET['k'])) {
			...
			$tknres = $GLOBALS['ISC_CLASS_DB']->Query($tkn);

			if (!$tkncust = $GLOBALS['ISC_CLASS_DB']->Fetch($tknres)) {
				return $this->ShowLoginPage("ActivateAccountInvalidLink", 1);
				die();
			}
			
			if ($tkncust['custnotes'] == "BLOQUEADO") {
				$this->ShowLoginPage("LockedUserMsj", 1);
				die();
			}
			else {
				...
				$entity = new ISC_ENTITY_CUSTOMER();
				if ($entity->edit($UpdateData, $tkncust['customerid'])) {
					return $this->LoginCustomerById($tkncust['customerid'], false);
				}
				else {
