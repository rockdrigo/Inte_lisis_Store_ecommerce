<?php
	...
	class ISC_BRANDS
	{
		...
		{	
			if ($this->_bprodperpage === "defaultpg") {
				if (GetConfig('CategoryProductsPerPage') > 0) {
					$this->_brandnumpages = ceil($this->GetNumProducts() / GetConfig('CategoryProductsPerPage'));
				}
				else {
					$this->_brandnumpages = 0;
				}
			}
			else	if ($this->_bprodperpage === "all") {
				$this->_brandnumpages = ceil($this->GetNumProducts() / $this->GetNumProducts());
			}
			else {
				$this->_brandnumpages = ceil($this->GetNumProducts() / $this->_bprodperpage);
			}
		}
			...
			switch ($this->_brandpage) {
				...
				default: {
					if ($this->_bprodperpage === "defaultpg") {
						...
						$start = ($this->GetPage() * $this->_bprodperpage) - $this->_bprodperpage;
					}
					break;
				}
			}
		...
		public function LoadProductsForBrand()
		{
			...
			if ($this->_bprodperpage === "defaultpg") {
				$onlyVar = GetConfig('CategoryProductsPerPage');
						...
						$onlyVar = $this->_bprodperpage;
					}
			$query = "
				SELECT p.*, FLOOR(prodratingtotal/prodnumratings) AS prodavgrating, pi.*, ip.Articulo, ".GetProdCustomerGroupPriceSQL()."
				FROM [|PREFIX|]products p
				LEFT JOIN [|PREFIX|]product_images pi ON (p.productid=pi.imageprodid AND pi.imageisthumb=1)
				LEFT OUTER JOIN [|PREFIX|]intelisis_products ip ON (p.productid=ip.productid)
				".$taxJoin."
				WHERE prodbrandid='".(int)$this->GetId()."' AND prodvisible='1'
				".GetProdCustomerGroupPermissionsSQL()."
				ORDER BY ".$this->GetSortField().", prodname ASC
			";
			$query .= $GLOBALS['ISC_CLASS_DB']->AddLimit($this->GetStart(), $onlyVar);
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
