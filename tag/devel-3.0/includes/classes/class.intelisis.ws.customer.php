...
class ISC_INTELISIS_WS_CUSTOMER extends ISC_INTELISIS_WS {
	...
	public function __construct($customerId, $customerData, $action) {
		...
		$this->customerData['custpassword'] = str_replace(array('â€�', '"', '&', 'Â°'), array('&#8221;', '&#34;', '&#38;', '&#176;'), $this->customerData['custpassword']);
		...
		if(!empty($customerData['addresses']))
		{
			...
			*/
			...
			$xml .= '<Direccion GUID="'.$customerData['addresses'][0]['GUID'].'" Direccion1="'.$customerData['addresses'][0]['shipaddress1'].'" Direccion2="'.$customerData['addresses'][0]['shipaddress2'].'" Ciudad="'.$customerData['addresses'][0]['shipcity'].'" Pais="'.$customerData['addresses'][0]['shipcountry'].'" Estado="'.$customerData['addresses'][0]['shipstate'].'" CP="'.$customerData['addresses'][0]['shipzip'].'" '.$customFieldsAddressXML.' />';
		}
