	...
	class ISC_ADMIN_REMOTE extends ISC_ADMIN_REMOTE_BASE
	{
		public function HandleToDo()
		{
			...
			switch  ($what) {
				...
				// REQ11162 JIB:
				case "sincroinfoquickview": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Products)) {
						$this->SincroInfoQuickView();
					}
					break;
				}
				case "generateapikey": {
		...
		private function mountTemplates(){
		...
		private function TestSMTPSettings()
		{
			...
			$email_api->Debug = true;
			$email_api->LogFile = ISC_BASE_PATH.'/email.debug.txt';
			$email_api->MemoryLogFile = ISC_BASE_PATH.'/email.mem.txt';
		...
		*/
		...
		/*REQ11162 JIB:
		 * Crea la funcion para obtener el xml completo
		 */
		private function SincroInfoQuickView()
		{
			if(!isset($_REQUEST['SincroId'])) {
				exit;
			}
			$query = sprintf("SELECT xml FROM [|PREFIX|]sincronizacion WHERE consecutivo='%d'", (int)$_REQUEST['SincroId']);
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$msg = $GLOBALS['ISC_CLASS_DB']->FetchOne($result);
