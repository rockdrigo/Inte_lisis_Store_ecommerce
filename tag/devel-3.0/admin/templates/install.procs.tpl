...
CREATE PROCEDURE `spAddIndex` (IN vTable CHAR(255), IN vColumn CHAR(255))
BEGIN
DECLARE vBase char(255);
SET vBase = DATABASE();

    SELECT COUNT(*) into @indexes
        FROM INFORMATION_SCHEMA.STATISTICS
        WHERE table_schema = vBase
        AND table_name = vTable
        AND COLUMN_NAME = vColumn;
  ...
  DECLARE CursorTerminado INT DEFAULT 0;     
   ...
   DECLARE crDescto CURSOR FOR    
    SELECT 
     ...
     WHERE ((IFNULL(p.ConVigencia,0) = 0) OR (pFechaEmision BETWEEN IFNULL(p.FechaD, FROM_UNIXTIME(0)) AND p.FechaA))
  ...
  SET vPrecioTemp = pPrecio;
   ...
   SET pDescuentoMonto = 0;
    SELECT 
     ...
     WHERE ((IFNULL(p.ConVigencia,0) = 0) OR (pFechaEmision BETWEEN IFNULL(p.FechaD, FROM_UNIXTIME(0)) AND p.FechaA)) -- Verifica la vigencia
       -- Condiciones de Propiedades de Articulo 
