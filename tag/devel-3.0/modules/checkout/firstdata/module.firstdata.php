<?php
	...
	class CHECKOUT_FIRSTDATA extends ISC_CHECKOUT_PROVIDER
	{
		...
		public function SetCustomVars()
		{
			...
			$this->_variables['transactiontype'] = array(
                ...
                $this->_variables['testmode'] = array("name" => "Test Mode",
                   "type" => "dropdown",
                   "help" => 'Modo de pruebas',
                   "default" => GetLang('FirstDataTestModeYes'),
                   "required" => true,
                   "options" => array(GetLang("FirstDataTestModeNo") => "NO",
                                                  GetLang("FirstDataTestModeYes") => "YES"
                        ),
                        "multiselect" => false
                );
		}
		...
		public function getAuthentication(){
			$authkey = base64_encode('WS'.$this->getValue('storeid').'._.1:'.$this->getValue('password'));
		...
		public function ProcessPaymentForm($dataSource = array())
		{
			...
			$xmlComplete = '<fdggwsapi:FDGGWSApiOrderRequest xmlns:v1="http://secure.linkpt.net/fdggwsapi/schemas_us/v1" xmlns:fdggwsapi="http://secure.linkpt.net/fdggwsapi/schemas_us/fdggwsapi">
				<v1:Transaction>
					...
					<v1:Payment>
						<v1:ChargeTotal>'.FormatNumber($this->GetGatewayAmount(), 2).'</v1:ChargeTotal>
						<v1:SubTotal>'.FormatNumber($this->GetSubTotal(), 2).'</v1:SubTotal>
						<v1:VATTax>'.FormatNumber($this->GetTaxCost(), 2).'</v1:VATTax>
						<v1:Shipping>'.FormatNumber($this->GetShippingCost(), 2).'</v1:Shipping>
					</v1:Payment>';
			...
			if($this->GetValue("testmode") == 'YES') {
				...
				$url = 'ws.firstdataglobalgateway.com';
			}
			...
			if (trim($result) != '') {
				...
				$transactionidResultDom = $dom->getElementsByTagName('TransactionResult');
				foreach($transactionidResultDom as $node){
					$transactionidResult = $node->nodeValue;
				}
				
				if($transactionidResult == 'DECLINED'){
					$this->SetError('Su tarjeta fue declinada');
					return false;
				}
		...
		private function ConnectToProvider($an_url, $an_pp_url, $an_data)
		{
			...
			if(function_exists("curl_exec")) {
				$certFile = ISC_BASE_PATH.'/config/'.$storeid.'/WS'.$storeid.'._.1.pem';
				$keyFile = ISC_BASE_PATH.'/config/'.$storeid.'/WS'.$storeid.'._.1.key';
				$keyPwd = file_get_contents(ISC_BASE_PATH.'/config/'.$storeid.'/WS'.$storeid.'._.1.key.pw.txt');
				...
				curl_setopt($ch, CURLOPT_USERPWD, 'WS'.$storeid.'._.1:'.$this->getValue('password'));
				curl_setopt($ch, CURLOPT_TIMEOUT, 60);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($ch, CURLOPT_POSTFIELDS, $an_data);
				curl_setopt($ch, CURLOPT_SSLCERT, $certFile);
				curl_setopt($ch, CURLOPT_SSLKEY, $keyFile);
				//curl_setopt($ch, CURLOPT_SSLCERTPASSWD, file_get_contents(ISC_BASE_PATH.'/config/'.$storeid.'/WS'.$storeid.'._.1.p12.pw.txt'));
				...
				if(!in_array($status, array('200', '202'))){
					$GLOBALS['ISC_CLASS_LOG']->LogSystemError(array('payment', $this->GetName()), 'Ocurrio un error en la autenticacion del modulo (Resultado de CURL regreso codigo "'.$status.'")');
					$this->SetError('Ocurrió un error al intentar contactar al proveedor de pago, porfavor vuelva a intentar.');
					return false;					
				}
