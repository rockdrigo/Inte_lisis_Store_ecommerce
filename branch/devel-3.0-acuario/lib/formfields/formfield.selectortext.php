...
{
	private $useOptionIndexAsValues = false;
	/**
	 * Constructor
	 *
	 * Base constructor
	...
	public function loadForFrontend()
	{
		...
		foreach ($options as $index => $val) {
			...
			if($this->useOptionIndexAsValues){
				$optionValue = $index;
			}
			else {
				$optionValue = isc_html_escape($val);
			}
			
			if (strtoupper($optionValue) == strtoupper((string)$this->value)) {
				$selected = 'selected="selected"';
			}

			$selectOptions .= '<option value="'.$optionValue.'" '.$selected.'>'.isc_html_escape($val).'</option>'."\n";
		}
	...
	*/
	...
	public function setOptionIndexAsValues($value){
		$this->useOptionIndexAsValues = $value;
	}
}
