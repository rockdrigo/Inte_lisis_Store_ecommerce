	...
	class ISC_PAGE
	{
		...
		*/
		public function _SetPageData($PageId=0)
		{
			...
			//REQ 11890 JIB: verifica que el usuario tenga permiso de entrar a la pagina
			if($row['pagecustomersonly'] == 2 && !$this->accessGroupPage()) {
				$GLOBALS['ErrorMessage'] = sprintf(GetLang('ForbiddenToAccessPage'), $GLOBALS['ShopPathNormal']);
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("error");
				$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
				exit;
		...
		//REQ 11890 JIB: verifica que el grupo del usuario tenga permiso de entrar a la pagina solicitada
		public function accessGroupPage() {
			$CustomerInfo = $GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerInfo();
			$groupId = $CustomerInfo['custgroupid'];
			$pageId = $this->GetPageId();
			$query = sprintf("select * from [|PREFIX|]pages_customer_groups where pageid='%d' and custgroupid='%d'", $pageId, $groupId);
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
			if($row == null){
				return false;
		...
		*/
		...
		{
			// JIB REQ11481: Agrege otros campos al formulario
			...
			if(!empty($_POST)) {
				...
				$GLOBALS['ContactNames'] = isc_html_escape($_POST['contact_names']);
				$GLOBALS['ContactLastName'] = isc_html_escape($_POST['contact_lastname']);
				$GLOBALS['ContactAddress'] = isc_html_escape($_POST['contact_address']);
				$GLOBALS['ContactColony'] = isc_html_escape($_POST['contact_colony']);
				$GLOBALS['ContactCountry'] = isc_html_escape($_POST['contact_country']);
				$GLOBALS['ContactCity'] = isc_html_escape($_POST['contact_city']);
				$GLOBALS['ContactZip'] = isc_html_escape($_POST['contact_zip']);
				$GLOBALS['ContactBirthday'] = isc_html_escape($_POST['contact_birthday']);
			...
			if(!is_numeric(isc_strpos($fields, "names"))) {
				$GLOBALS['HideNames'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "lastname"))) {
				$GLOBALS['HideLastName'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "address"))) {
				$GLOBALS['HideAddress'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "colony"))) {
				$GLOBALS['HideColony'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "country"))) {
				$GLOBALS['HideCountry'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "city"))) {
				$GLOBALS['HideCity'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "zip"))) {
				$GLOBALS['HideZip'] = "none";
			}
			
			if(!is_numeric(isc_strpos($fields, "birthday"))) {
				$GLOBALS['HideBirthday'] = "none";
			}
		...
		*/
		public function SendContactForm()
		{
			...
			if($captcha_check) {
				...
				if(is_numeric(isc_strpos($fields, "names"))) {
					$GLOBALS['FormField'] = GetLang('ContactNames');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_names']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "lastname"))) {
					$GLOBALS['FormField'] = GetLang('ContactLastName');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_lastname']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "address"))) {
					$GLOBALS['FormField'] = GetLang('ContactAddress');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_address']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "colony"))) {
					$GLOBALS['FormField'] = GetLang('ContactColony');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_colony']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "country"))) {
					$GLOBALS['FormField'] = GetLang('ContactCountry');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_country']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "city"))) {
					$GLOBALS['FormField'] = GetLang('ContactCity');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_city']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "zip"))) {
					$GLOBALS['FormField'] = GetLang('ContactZip');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_zip']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
				
				if(is_numeric(isc_strpos($fields, "birthday"))) {
					$GLOBALS['FormField'] = GetLang('ContactBirthday');
					$GLOBALS['FormValue'] = isc_html_escape($_POST['contact_birthday']);
					$GLOBALS['FormFieldList'] .= $emailTemplate->GetSnippet("ContactFormField");
				}
