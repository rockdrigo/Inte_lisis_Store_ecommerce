...
class ISC_INTELISIS_ECOMMERCE_WEBART extends ISC_INTELISIS_ECOMMERCE
{
	...
	private function createProduct() {
		...
		// NES - Esta bandera es solo para Acuario, pero se diseña la columna "clasifier" de cada producto para poder expandir esta funcionalidad.
		// Los clasificadores son como categorias, pero para uso interno, no se muestran al cliente (menos en Panel.CartContentClasifiers que es especifico para Acuario)
		if($this->getData('EsAlimento') == '1'){
			$clasifier = 'Alimentos';
		}
		elseif($this->getData('EsAlimento') == '0'){
			$clasifier = 'Accesorios';
		}
		else {
			$clasifier = '';
		}
	...
	private function updateProduct() {
		...
		if(!$GLOBALS['ISC_CLASS_DB']->UpdateQuery('products', $productData, 'productid = "'.$productId.'"'))
		{
			...
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery('intelisis_products', array('clasifier' => $clasifier));
