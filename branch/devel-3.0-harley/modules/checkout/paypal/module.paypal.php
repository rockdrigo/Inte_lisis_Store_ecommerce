...
class CHECKOUT_PAYPAL extends ISC_CHECKOUT_PROVIDER
{
	...
	public function SetCustomVars()
	{
		...
		$this->_variables['multiaccount'] = array("name" => GetLang('PayPalMultiAccount'),
				"type" => "checkbox",
				"help" => GetLang('PayPalMultiAccountHelp'),
				"default" => "",
				"required" => true,
				"javascript" => $this->ParseTemplate('property.multiaccount.js', true)
		);
		
		$this->_variables['account_table'] = array("name" => GetLang('PayPalAccounts'),
				'type' => 'custom',
				'callback' => 'BuildAccountsForm',
				'default' => '',
				'required' => false,
				//"help" => GetLang('PayPalAccountsHelp'),
				'javascript' => '',
		);
	}
	
	private function _buildStoreOptions($selected = ''){
		if(trim($selected) == '') $selectedOption = 'selected="selected"';
		else $selectedOption = '';
		$storeoptions = '<option '.$selectedOption.' value="">-- Seleccione una Sucursal --</option>';
		
		$result = $GLOBALS['ISC_CLASS_DB']->Query('SELECT Sucursal, Nombre FROM [|PREFIX|]intelisis_Sucursal WHERE eCommerceVentaEmpresa IS NOT NULL');
		
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
			if(trim($selected) != '' && $selected == $row['Sucursal']) $selectedOption = 'selected="selected"';
			else $selectedOption = ''; 
			$storeoptions .= '<option '.$selectedOption.' value="'.$row['Sucursal'].'">'.$row['Nombre'].'</option>';
		}
		return $storeoptions;
	}
	...
	public function getpropertiessheet($tab_id)
	{
		...
		$output = $this->ParseTemplate('accounts_header', true);
	
		$output .= parent::getpropertiessheet($tab_id);
		return $output;
	}
	
	public function BuildAccountsForm(){
		...
		if (empty($this->accounts)) {
			$GLOBALS['POS'] = 0;
			$GLOBALS['STOREOPTIONS'] = $this->_buildStoreOptions();
			return $this->ParseTemplate('accounts_row', true);
		}
		
		$output = '';
		foreach ($this->accounts as $id => $account) {
			$GLOBALS['POS'] = $id;
			$GLOBALS['ACCOUNTNAME_VAL'] = $account['accountname'];
			$GLOBALS['STOREOPTIONS'] = $this->_buildStoreOptions($account['accountstore']);
		
			$output .= $this->ParseTemplate('accounts_row', true);
		}
		return $output;
	}
	...
	private function LoadAccounts()
	{
		$query = "SELECT variablename, variableval
			...
			WHERE modulename='checkout_paypal'
			AND (
				variablename LIKE 'accountname_%'
				OR variablename LIKE 'accountstore_%'
			)";
		...
		while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			if($name == 'accountname') $this->accounts[$pos]['accountname'] = $row['variableval'];
			if($name == 'accountstore') $this->accounts[$pos]['accountstore'] = $row['variableval'];
		}
	
		return $this->accounts;
	}
	
	private function getAccountEmail(){
		if(!$this->GetValue('multiaccount') || !GetConfig('UsePreCart')){
			return $this->getValue('email');
		}
		
		if(isset($_SESSION['STOREORIGIN']) && is_int($_SESSION['STOREORIGIN'])){
			foreach($this->accounts as $account){
				if($account['accountstore'] == $_SESSION['STOREORIGIN'])
