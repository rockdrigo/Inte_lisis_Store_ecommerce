...
class ISC_PREQUOTE_ITEM
{
	...
	*/
	...
	/**
	 * @var ISC_PREQUOTE ISC_PREQUOTE instance this item belongs to.
	 */
	protected $quote;
	...
	/**
	 * @var string ID of the ISC_PREQUOTE_ADDRESS object this item should be shipped
	 * to.
	 */
	 ...
	 * @return ISC_PREQUOTE_ITEM This item instance.
	 */
	 ...
	 * @return ISC_PREQUOTE_ITEM Current item instance.
	 */
	...
	*/
	 ...
	 *
	 * @return ISC_PREQUOTE Instance of the quote object the product was attached
	 * to.
	 */
	 ...
	 *
	 * @return ISC_PREQUOTE Instance of the ISC_PREQUOTE object.
	 */
	 ...
	 *
	 * @param ISC_PREQUOTE $quote Quote this item should belong to.
	 * @return ISC_PREQUOTE_ITEM This item instance.
	 */
	public function setQuote(ISC_PREQUOTE $quote)
	{
		$this->quote = $quote;
		return $this;
	}
	...
	public function addChildItem(ISC_PREQUOTE_ITEM $item)
	{
	 ...
	 *
	 * @return ISC_PREQUOTE_ITEM Current quote item.
	 */
	...
	public function getConfigurableOptions()
	{
		...
		if(checkForStoreOriginNecessary($this->getProductId())){
			$configurableFields[0] = array (
				"productfieldid"	=> 0,
				"fieldname"			=> 'Sucursal',
				"fieldtype"			=> 'select',
				"fileType"		=> '*',
				"fileSize"		=> '',
				"fieldselectoptions"	=> getStoreOriginOptions($this->getProductId()),
				"fieldrequired"		=> 1,
				"fieldlayer"			=> '',
			);
		}
	...
	public function getEventDateAsArray($timestamp = false)
	{
		return $this->eventDate;
	}
	 ...
	 * @return ISC_PREQUOTE_ITEM This quote item instance.
	 */
	...
	*/
	public function getUnallocatedQuantity()
	{
		...
		$unallocated = $this->quote->getAddressById(ISC_PREQUOTE_ADDRESS::ID_UNALLOCATED);
		if (!$unallocated) {
			return 0;
		}
	...
	*
	* @param string $id can also be supplied as ISC_PREQUOTE_ADDRESS instance from which the id will be taken
	* @return ISC_PREQUOTE_ITEM
	*/
	public function setAddressId($id)
	{
		if ($id instanceof ISC_PREQUOTE_ADDRESS) {
	...
	* @return ISC_PREQUOTE_ITEM returns the instance of the item at it's new location (meaning, if the item is cloned the NEW instance is returned)
	*/
	public function moveToAddress(ISC_PREQUOTE_ADDRESS $address, $quantity = null)
	{
	...
	*/
	...
	public function afterAddedtoCart() {
		...
		if(GetConfig('isIntelisis')){
			$newprice = applyPyC($this->getProductData(), $this->getQuantity(), $this->getVariationId());
	...
	/**
	 * Asigna la Sucursal de Origen del producto. En orden de preferencia:
	 * 1) El campo configurable elegido con el producto. Este se muestra condicionalmente, así que no siempre se puede escoger.
	 * 2) La Sucursal asignada del Cliente. Si es cliente anonimo no se considera esta opcion
	 * 3) La sucursal por omision asignada a la Tienda. 
	 * 
	 * @return boolean TRUE si se pudo asignar una Sucursal de Origen, FALSE si en la Sucursal elegida, asignada o por omisión no hay existencias.
	 */
	public function setStoreOrigin(){
		// El campo configurable de Sucursal
		$configuration = $this->getConfiguration();
		$StoreOriginId = false;
		if(array_key_exists(0, $configuration)){
			$StoreOriginId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT Sucursal FROM [|PREFIX|]intelisis_Sucursal WHERE Nombre = "'.$configuration[0]['value'].'"', 'Sucursal');
			unset($this->configuration[0]);
		}

		if(checkProductStockStoreOrigin($this->productData['prodcode'], $StoreOriginId)){
			$this->StoreOrigin = $StoreOriginId;
			return true;
		}
		
		// Cliente logeado pero no eligio campo configurable. Sucursal por omision del Cliente.
		if(CustomerIsSignedIn() && !$StoreOriginId){
			$StoreOriginId = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT Sucursal FROM [|PREFIX|]intelisis_customers WHERE customerid = "'.$GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId().'"');
		}
		...
		// La Sucursal por omision
		if(!$StoreOriginId){
			$StoreOriginId = getDefaultStoreId();
		}
		...
		// Ninguna de las tres opciones regreso una Sucursal con existencias. Regresar error.
		return false;
	}
