<?php
	...
	class ISC_BRANDS
	{
		...
		public function LoadProductsForBrand()
		{
			...
			$query = "
				SELECT p.*, FLOOR(prodratingtotal/prodnumratings) AS prodavgrating, pi.*, ".GetProdCustomerGroupPriceSQL().GetIntelisisStockSQL()."
				FROM [|PREFIX|]products p
				LEFT JOIN [|PREFIX|]product_images pi ON (p.productid=pi.imageprodid AND pi.imageisthumb=1)
				".$taxJoin;
			
			if(GetConfig('UsePreCart')){
				$query .= 'LEFT JOIN [|PREFIX|]intelisis_inv ii ON (p.prodcode = ii.SKU)';
			}
			
			$query .= " WHERE prodbrandid='".(int)$this->GetId()."' AND prodvisible='1'
				".GetProdCustomerGroupPermissionsSQL();
			...
			$query .= " ORDER BY ".$this->GetSortField().", prodname ASC
			";
