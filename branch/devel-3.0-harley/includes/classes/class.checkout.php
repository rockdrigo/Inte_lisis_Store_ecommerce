...
class ISC_CHECKOUT
{
	...
	public function HandlePage()
	{
		...
		if((!GetConfig('UsePreCart') && $this->getQuote()->getNumItems() == 0) || !(bool)GetConfig('AllowPurchasing') || !(bool)GetConfig('ShowProductPrice')) {
	...
	private function checkPreCart(){
		if(GetConfig('UsePreCart') && isset($_GET['storeorigin']) && is_numeric($_GET['storeorigin'])){
			EmptyCartAndKillCheckout();
			$quote = getCustomerQuote();
			$prequote = getCustomerPreQuote();
			foreach($prequote->getItemsByStore(null, $_GET['storeorigin']) as $itemPreQuote){
				try {
					$item = new ISC_QUOTE_ITEM;
					$item
						->setQuote($quote)
						->setProductId($itemPreQuote->getProductId())
						->setQuantity($itemPreQuote->getQuantity())
						->setVariationId($itemPreQuote->getVariationId())
						->setVariationOptions($itemPreQuote->getVariationOptions())
						->applyConfiguration($itemPreQuote->getConfiguration())
						->afterAddedtoCart();
					$EventDate = $itemPreQuote->getEventDateAsArray();
					if(!empty($EventDate)) {
						...
						$item
							->setEventDate(
								...
								$EventDateArray['Day'],
								$EventDateArray['Yr'])
							->setEventName($itemPreQuote->getEventName());
					}
					
					$this->getQuote()->addItem($item);
				}
				catch(ISC_QUOTE_EXCEPTION $e){
					logAddNotice('hrtgfhrf');
					break;
				}
			}
			$_SESSION['STOREORIGIN'] = $_GET['storeorigin'];
		}
	}
	...
	public function BuildOrderConfirmation()
	{
		...
		foreach ($items as $item) {
			...
			if (!empty($configuration)) {
				logAddNotice(serialize($configuration));
