...
class ISC_PREQUOTE
{
	...
	/**
	 * @var array Array containing ISC_PREQUOTE_ITEM instances for items in the quote.
	 */
	protected $items = array();
	
	/**
	 * @var array Array containing ISC_PREQUOTE_ITEM instances for items in the quote, inside arrays of StoreOrigin Id's.
	 */
	protected $itemsByStore = array();
	...
	* usort($items, array('ISC_PREQUOTE', 'sortItemsByAddressIdCallback'));
	*
	* @param ISC_PREQUOTE_ITEM $a
	* @param ISC_PREQUOTE_ITEM $b
	* @return int
	*/
	...
	*
	* This calculation was originally performed in a few places. It seemed logical to move it to ISC_PREQUOTE so Twig templates can access it via quote objects.
	*
	* @return bool
	*/
	 ...
	 * @return ISC_PREQUOTE This quote instance.
	 */
	 ...
	 *
	 * @return ISC_PREQUOTE_ADDRESS ISC_PREQUOTE_ADDRESS instance w/ billing details.
	 */
	public function getBillingAddress()
	{
		foreach ($this->addresses as $address) {
			if ($address->getType() == ISC_PREQUOTE_ADDRESS::TYPE_BILLING) {
				return $address;
			}
		}

		// No billing address was found. Silently create one
		$address = new ISC_PREQUOTE_ADDRESS;
	 ...
	 *
	 * @param ISC_PREQUOTE_ADDRESS $address ISC_PREQUOTE_ADDRESS instance.
	 * @return ISC_PREQUOTE This quote instance.
	 */
	public function setBillingAddress(ISC_PREQUOTE_ADDRESS $address)
	{
		$address->setQuote($this);
		$address->setType(ISC_PREQUOTE_ADDRESS::TYPE_BILLING);
	 ...
	 * on the quote.
	 *
	 * @return ISC_PREQUOTE_ADDRESS_SHIPPING Quote shipping address instance.
	 */
	public function getShippingAddress()
	{
		foreach ($this->addresses as $address) {
			if ($address->getType() == ISC_PREQUOTE_ADDRESS::TYPE_SHIPPING) {
				return $address;
			}
		}

		// Still here? Create an address siliently
		$address = new ISC_PREQUOTE_ADDRESS_SHIPPING;
	...
	*/
	public function removeShippingAddress($addressId, $moveItemsTo = null)
	{
		...
		foreach ($this->addresses as $key => $address) {
			...
			if ($address->getId() == $addressId) {
				...
				foreach ($items as /** @var ISC_PREQUOTE_ITEM */$item) {
	...
	/**
	* Array sorting callback method to place the 'unallocated' ISC_PREQUOTE_ADDRESS first as sometimes it may be created after the other addresses (e.g. when editing an order)
	*
	* @param ISC_PREQUOTE_ADDRESS $a
	* @param ISC_PREQUOTE_ADDRESS $b
	* @return int
	*/
	public static function sortUnallocatedAddressFirst(ISC_PREQUOTE_ADDRESS $a, ISC_PREQUOTE_ADDRESS $b)
	{
	 ...
	 *
	 * @return array Array of ISC_PREQUOTE_ADDRESS_SHIPPING instances.
	 */
	public function getShippingAddresses()
	{
		...
		usort($shippingAddresses, array('ISC_PREQUOTE', 'sortUnallocatedAddressFirst'));
	 ...
	 *
	 * @return ISC_PREQUOTE_ADDRESS Address instance with given ID or false on failure
	 */
	 ...
	 * quote.
	 *
	 * @param ISC_PREQUOTE_ADDRESS_SHIPPING $address Shipping address instance.
	 * @return ISC_PREQUOTE This quote instance.
	 */
	public function addShippingAddress(ISC_PREQUOTE_ADDRESS_SHIPPING $address)
	{
		$address->setQuote($this);
		$address->setType(ISC_PREQUOTE_ADDRESS::TYPE_SHIPPING);
		if ($this->isSplitShipping == false) {
			foreach ($this->addresses as $k => $existingAddress) {
				if ($existingAddress->getType() == ISC_PREQUOTE_ADDRESS::TYPE_SHIPPING) {
	...
	/**
	 * Given an ISC_PREQUOTE_ITEM instance, add it to this quote.
	 *
	 * @param ISC_PREQUOTE_ITEM $item ISC_PREQUOTE_ITEM instance to add.
	 * @param boolean Attempt to increment quantities if this item already exists or not.
	 * @return ISC_PREQUOTE This quote instance.
	 */
	public function addItem(ISC_PREQUOTE_ITEM $item, $collapse = true)
	{
		$hash = $item->getHash();
		if ($collapse != false) {
		...
		try {
			...
			if(!isset($this->itemsByStore[$item->getStoreOrigin()]))
				$this->itemsByStore[$item->getStoreOrigin()] = array();
			$this->itemsByStore[$item->getStoreOrigin()][] = $item;
			
			$item->setInQuote(true);
		}
		catch(ISC_PREQUOTE_EXCEPTION $e) {
			// Remove the item from the quote.
			array_pop($this->items);
			array_pop($this->items[$item->getStoreOrigin()]);
	 ...
	 * @return ISC_PREQUOTE This instance of ISC_PREQUOTE.
	 */
	public function removeItem($itemId)
	{
		...
		foreach ($this->itemsByStore as $StoreId => $items) {
			foreach ($items as $key => $item) {
				if ($itemId == $item->getId()) {
					$item->removeChildren();
					unset($this->itemsByStore[$StoreId][$key]);
					break;
				}
			}
		}
	 ...
	 * quote.
	 *
	 * @param ISC_PREQUOTE_ITEM $item Optional item, to ensure it is also loaded.
	 * @return ISC_PREQUOTE This instance of ISC_PREQUOTE.
	 */
	public function loadProductData(ISC_PREQUOTE_ITEM $additionalItem = null)
	{
		$productIds = array();
		$variationIds = array();
	 ...
	 * item is its instance of the ISC_PREQUOTE_ITEM object.
	 *
	 * @param int $type optional, limit returned items to one of PT_? specifiers
	 * @return array Array of ISC_PREQUOTE_ITEM objects for this quote.
	 */
	public function getItems($type = null)
	{
		...
		$items = array();
		foreach ($this->items as /** @var ISC_PREQUOTE_ITEM */$item) {
	...
	/**
	 * Return an array of all of the items in this quote, sorted by StoreOriginId. Each returned
	 * item is its instance of the ISC_PREQUOTE_ITEM object.
	 *
	 * @param int $type optional, limit returned items to one of PT_? specifiers
	 * @return array Array of ISC_PREQUOTE_ITEM objects for this quote.
	 */
	public function getItemsByStore($type = null, $StoreOriginId = null)
	{
		if ($type === null && $StoreOriginId === null) {
			return $this->itemsByStore;
		}
		
		if($type === null && is_numeric($StoreOriginId)){
			if(isset($this->itemsByStore[$StoreOriginId]))
				return $this->itemsByStore[$StoreOriginId];
			else 
				return array();
		}
	 ...
	 * @return ISC_PREQUOTE_ITEM False on fail, otherwise item instance.
	 */
	 ...
	 */
	public function getSubTotal($StoreOriginId, $incTax = false)
	{
		$subtotal = 0;
		foreach ($this->itemsByStore[$StoreOriginId] as $item) {
	...
	public function getDiscountedSubTotal($StoreOriginId, $incTax)
	{
		$subtotal = 0;
		foreach ($this->itemsByStore[$StoreOriginId] as $item) {
	...
	public function getGrandTotal($StoreOriginId)
	{
		$totals = array(
			'subtotal' => $this->getDiscountedSubTotal($StoreOriginId, true),
			'wrapping' => $this->getWrappingCost(true),
			'shipping' => $this->getShippingCost(true),
			'handling' => $this->getHandlingCost(true),
			'giftCertificates' => $this->getGiftCertificateTotal($StoreOriginId) * -1,
		);
	...
	public function getGrandTotalWithoutGiftCertificates($StoreOriginId)
	{
		$totals = array(
			'subtotal' => $this->getDiscountedSubTotal($StoreOriginId, true),
			'wrapping' => $this->getWrappingCost(true),
	 ...
	 *
	 * @return array Array of ISC_PREQUOTE_ADDRESS instances belong to this quote.
	 */
	public function getAllAddresses()
	{
		...
		foreach ($this->addresses as $address) {
			if ($address instanceof ISC_PREQUOTE_ADDRESS_SHIPPING) {
				continue;
			}
	...
	*/
	...
	public function applyCoupon($coupon)
	{
		...
		if(is_string($coupon) || is_numeric($coupon)) {
			...
			if (!$coupon || !$coupon['couponcode']) {
				throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponCode'), ISC_PREQUOTE_EXCEPTION::COUPON_INVALID);
			}
		...
		if (isset($coupon['couponenabled']) && $coupon['couponenabled'] == 0) {
			throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponDisabled'), ISC_PREQUOTE_EXCEPTION::COUPON_DISABLED);
		}
		...
		if (isset($coupon['couponmaxuses']) && $coupon['couponnumuses'] && $coupon['couponmaxuses'] != 0 && $coupon['couponnumuses'] >= $coupon['couponmaxuses']) {
			throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponExpiredUses'), ISC_PREQUOTE_EXCEPTION::COUPON_EXPIRED_USES);
			return false;
		}
		...
		if (isset($coupon['couponminpurchase']) && $coupon['couponminpurchase'] > 0 && $subtotal < $coupon['couponminpurchase']) {
			...
			throw new ISC_PREQUOTE_EXCEPTION($invalidCouponMesg, ISC_PREQUOTE_EXCEPTION::COUPON_MIN_PURCHASE);
		}
		...
		if(!$appliesToQuote) {
			if ($shippingLocationUnapplied) {
				throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponLocation'), ISC_PREQUOTE_EXCEPTION::COUPON_LOCATION_DOES_NOT_APPLY);
			}
			else if ($shippingMethodUnapplied) {
				throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponMethod'), ISC_PREQUOTE_EXCEPTION::COUPON_METHOD_DOES_NOT_APPLY);
			}
			else {
				throw new ISC_PREQUOTE_EXCEPTION(GetLang('InvalidCouponCode'), ISC_PREQUOTE_EXCEPTION::COUPON_DOES_NOT_APPLY);
			}
		}
	 ...
	 * by the address.
	 *
	 * @param ISC_PREQUOTE_ADDRESS_SHIPPING $shippingAddress The shipping address
	 * @param int $couponId The coupon id
	...
	protected function _isCouponValidByLocation($shippingAddress, $couponId)
	{
		...
		if (!$address['shipzip']) {
			...
			throw new ISC_PREQUOTE_EXCEPTION(GetLang('CouponLocationNotSpecified'), ISC_PREQUOTE_EXCEPTION::COUPON_LOCATION_DOES_NOT_SPECIFIED);
		} else {
			$address = $shippingAddress->getAsArray();
	 ...
	 *
	 * @throws ISC_PREQUOTE_EXCEPTION when the gift certificate cannot be applied
	...
	public function applyGiftCertificate($code)
	{
		...
		if (!$certificate['giftcertid'] || $certificate['giftcertbalance'] == 0) {
			throw new ISC_PREQUOTE_EXCEPTION(getLang('BadGiftCertificateInvalid'));
		}
	...
	public function getAppliedGiftCertificates($StoreOriginId)
	{
		uasort($this->giftCertificates, 'giftCertificateSort');
		$giftCertificates = array();

		$runningTotal = $this->getGrandTotalWithoutGiftCertificates($StoreOriginId);
		if ($runningTotal == 0) {
	...
	public function getGiftCertificateTotal($StoreOriginId)
	{
		$giftCertificates = $this->getAppliedGiftCertificates($StoreOriginId);
	...
	*/
	...
	/**
	* Creates a new ISC_PREQUOTE_ITEM instance for this ISC_PREQUOTE and returns it. Note: the item is not added to the quote, you must still call ISC_PREQUOTE->addItem
	*
	* @return ISC_PREQUOTE_ITEM
	*/
	...
	/**
	* Creates a new ISC_PREQUOTE_ADDRESS_SHIPPING instance for this ISC_PREQUOTE and returns it.
	*
	* @param string $id specific id to create this address with otherwise leave it to random id generator
	* @return ISC_PREQUOTE_ADDRESS_SHIPPING
	*/
	...
	*
	* @param ISC_PREQUOTE_ADDRESS $address
	* @param array $items as an array of quote item id => quantity or use null as quantity to move all
	* @return ISC_PREQUOTE
	*/
	public function moveItems(ISC_PREQUOTE_ADDRESS $address, $items)
	{
	 ...
	 *
	 * @return ISC_PREQUOTE Return an object of ISC QUOTE
	 */
	...
	public function removeItemsByStore($StoreOrigin){
		unset($_SESSION['PREQUOTE']->itemsByStore[$StoreOrigin]);
		foreach($_SESSION['PREQUOTE']->items as $index => $item){
			if($item->getStoreOrigin() == $StoreOrigin)
				unset($_SESSION['PREQUOTE']->items[$index]);
		}
		unset($_SESSION['STOREORIGIN']);
	}
