...
class ISC_INTELISIS_WS_ORDER extends ISC_INTELISIS_WS {
	...
	public function __construct($orderid) {
		...
		$Empresa = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT eCommerceVentaEmpresa FROM [|PREFIX|]intelisis_Sucursal WHERE Sucursal = "'.$_SESSION['STOREORIGIN'].'"', 'eCommerceVentaEmpresa');
		if($Empresa)
		...
		foreach($pedidoD as $D) {
			...
			if(!empty($D['CamposConfigurables'])){
				...
				foreach($D['CamposConfigurables'] as $key => $field) {
					$line .= '<CampoConfigurable Campo="'.$field['Nombre'].'" Valor="'.$field['Valor'].'" />';
				}
				$line .= '</CamposConfigurables>';
			}
