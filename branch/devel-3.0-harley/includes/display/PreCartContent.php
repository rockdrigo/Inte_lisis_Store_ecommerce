<?php
class ISC_PRECARTCONTENT_PANEL extends PANEL
{
	protected $quote;
	...
	{
		$this->quote = getCustomerPreQuote();
	...
	public function generateCartContent()
	{
		...
		$GLOBALS['SNIPPETS']['PreCartItemGroups'] = "";

		$deliveryDates = array();
		$itemsPreCart = $this->quote->getItemsByStore();
		
		foreach($itemsPreCart as $StoreOriginId => $items){
			if(empty($items))
				continue;
			$GLOBALS['StoreOriginId'] = $StoreOriginId;
			...
			$GLOBALS['PreCartGroupItems'] = '';
			$StoreOriginName = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT Nombre FROM [|PREFIX|]intelisis_Sucursal WHERE Sucursal = "'.$StoreOriginId.'"', 'Nombre');
			$GLOBALS['PreCartItemsForStoreOrigin'] = sprintf(GetLang('PreCartItems'), $StoreOriginName);
			foreach($items as $item) {
				...
				$GLOBALS['PreCartGroupItems'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("CartItem");
			...
			// Cambiar estas funciones para que acepten el StoreOrigin
			$GLOBALS['CartItemTotal'] = currencyConvertFormatPrice($this->quote->getSubTotal($StoreOriginId, $this->displayIncludingTax));
			$GLOBALS['CartTotal'] = currencyConvertFormatPrice($this->quote->getGrandTotal($StoreOriginId));
			...
			$GLOBALS['SNIPPETS']['PreCartItemGroups'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("PreCartItemGroup");
