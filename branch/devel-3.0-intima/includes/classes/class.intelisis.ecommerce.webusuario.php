...
class ISC_INTELISIS_ECOMMERCE_WEBUSUARIO extends ISC_INTELISIS_ECOMMERCE {
	...
	private function createCustomer() {
		...
		if(GetConfig('ForcePasswordChangeNewUsers')){
				$customerData['custnotes'] = 'PASSWORD';
		}
		...
		// Else is the provided phone number valid?
			...
			if (isId($customerId)) {
				if(!($rtn_insert = $GLOBALS['ISC_CLASS_DB']->InsertQuery('intelisis_customers', array('GUID' => $this->getData('GUID'), 'customerid'=>$customerId, 'Cliente'=>$this->getData('Cliente'), 'IDWebUsuario' => $this->getData('ID')), true)))
				{
	...
	private function updateCustomer() {
			...
			if(GetConfig('ForcePasswordChangeNewUsers')){
				$UpdatedAccount['custnotes'] = 'PASSWORD';
			}
			...
			{
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery('intelisis_customers', array('Cliente' => $this->getData('Cliente'), 'customerid' => $customer_id), 'GUID = "'.$this->getData('GUID').'"');
