...
/*REQ9584 JIB:
 * Clase para hacer el menu de categorias de forma horizontal y que sea independiente al menu de categorias vertical
 */

	CLASS ISC_TOPCATEGORYLIST_PANEL extends PANEL
	{
		public function SetPanelSettings()
		{
			if (ISC_CATEGORY::areCategoryFlyoutsEnabled()) {
				$GLOBALS['TopCategoryListTypeClass'] = 'TopCategoryListFlyout';
				$output = $this->_generateFlyoutOutput();
			} else {
				$GLOBALS['TopCategoryListTypeClass'] = 'TopCategoryListClassic';
			...
			$GLOBALS['SNIPPETS']['TopCategoryList'] = $output;
		}
		...
		*/
		protected function _getSubCategory($categories, $parentCatId)
		{
			...
			if (isset($categories[$parentCatId]) && !empty($categories[$parentCatId])) {
				...
				foreach ($categories[$parentCatId] as $subCat) {
					...
					$output .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("TopCategoryList");
				}
			}
			if ($output!='') {
		...
		protected function _generateClassicOutput ()
		{
			...
			foreach($categories[0] as $rootCat) {
				...
				// @todo ul here is hacky but front end templates are limited, fix this when possible
				$output .= '<ul>' . $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("TopCategoryList") . '</ul>';
			}
		...
		protected function _generateFlyoutOutput ()
		{
			...
			return $renderer->setSiteMapTree($categories)
				->setRootClasses('sf-menu sf-horizontal')
				->render();
		}
	}
