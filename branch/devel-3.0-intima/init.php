...
$GLOBALS['RewriteRules'] = array(
	...
	"offers" => array(
		"class" => "class.offers.php",
		"name" => "ISC_OFFERS",
		"global" => "ISC_CLASS_OFFERS"
	)
);
...
if (isset($_SERVER['REQUEST_URI']) && ($_SERVER['REQUEST_URI'] != "/login.php?action=reset_password")) {
	if ($customerInfo) {
		if($customerInfo['custnotes'] == "PASSWORD") {
			logAddNotice('Location: '.getConfig('ShopPathNormal').'/login.php?action=reset_password');
			header('Location: '.getConfig('ShopPathNormal').'/login.php?action=reset_password');
		}
	}
}
