<?php
	...
	class CHECKOUT_PAYPALEXPRESS extends ISC_CHECKOUT_PROVIDER
	{
		...
		*/
		 ...
		 **/
		public function GetCheckoutButton1()
		{
		...
		public function GetSidePanelCheckoutButton1()
		{
		...
		*/
		private function SetExpressCheckout()
		{
			...
			if(CustomerIsSignedIn()){
				...
				$useremail = $custinfo['custconemail'];
			}
			...
			if(isset($_COOKIE['SHOP_ORDER_TOKEN'])) {
				...
				if($order['ordisdigital']) {
					...
					$shippingDetails = array (
						'PAYMENTREQUEST_0_SHIPTONAME' => $shippingAddress['first_name']." ".$shippingAddress['last_name'],
						'PAYMENTREQUEST_0_SHIPTOSTREET' => $shippingAddress['address_1'],
						'PAYMENTREQUEST_0_SHIPTOSTREET2' => $shippingAddress['address_2'],
						'PAYMENTREQUEST_0_SHIPTOCITY' => $shippingAddress['city'],
						'PAYMENTREQUEST_0_SHIPTOZIP' => $shippingAddress['zip'],
						'PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE' => $shippingAddress['country_iso2'],
						'PAYMENTREQUEST_0_SHIPTOPHONENUM' => $shippingAddress['phone'],
							
						'L_SHIPPINGOPTIONISDEFAULT0' => 1,
						'L_SHIPPINGOPTIONNAME0' => 'Fijo',
						'L_SHIPPINGOPTIONAMOUNT0' => $this->GetShippingCost(),
					);

					if($shippingAddress['state_id'] != 0 && GetStateISO2ById($shippingAddress['state_id'])) {
						$shippingDetails['PAYMENTREQUEST_0_SHIPTOSTATE'] = GetStateISO2ById($shippingAddress['state_id']);
					}
					else {
						$shippingAddress['PAYMENTREQUEST_0_SHIPTOSTATE'] = isc_html_escape($shippingAddress['state']);
					}
				}
				...
				$addressOverride = 0;
				$shippingDetails['L_SHIPPINGOPTIONISDEFAULT0'] = 1;
				$shippingDetails['L_SHIPPINGOPTIONNAME0'] = 'Fijo';
				$shippingDetails['L_SHIPPINGOPTIONAMOUNT0'] = 0;
			}
			...
			$pp_array = array(
				...
				'PAYMENTREQUEST_0_PAYMENTACTION'		=> $merchant['transactionType'],
				'MAXAMT'					=> $amount,

				'RETURNURL'		=> $GLOBALS['ShopPath']."/finishorder.php",
//$GLOBALS['ShopPath']."/checkout.php?action=set_external_checkout&provider=".$this->GetId(),
				'CANCELURL'		=> $GLOBALS['ShopPath']."/cart.php",
				'ADDROVERRIDE'	=> 1,
				'CALLBACK'		=> $GLOBALS['ShopPath'].'/checkout.php?action=gateway_ping&provider='.$this->GetId(),
				'CALLBACKTIMEOUT' => 6,
				'L_PAYMENTREQUEST_0_NAME0 '		=> getLang('YourOrderFromX', array('storeName' => getConfig('StoreName'))),
		...
		*
		*/
		public function DoExpressCheckoutPayment1111()
		{
			if(isset($_COOKIE['SHOP_ORDER_TOKEN'])) {
				...
				$pp_array = array(
					...
					'PAYMENTREQUEST_0_AMT'			=> number_format($quote->getGrandTotal(),2,'.',''),
					'PAYMENTREQUEST_0_CURRENCYCODE'		=> $currency,
					'PAYMENTREQUEST_0_PAYMENTACTION'		=> $merchant['transactionType'],
					
					'RETURNURL'		=> $GLOBALS['ShopPath']."/checkout.php?action=set_external_checkout&provider=".$this->GetId(),
					'CANCELURL'		=> $GLOBALS['ShopPath']."/cart.php",
					'ADDROVERRIDE'	=> 1,
					'CALLBACK'		=> $GLOBALS['ShopPath'].'/checkout.php?action=gateway_ping&provider='.$this->GetId(),
					'L_PAYMENTREQUEST_0_NAME1 '		=> getLang('YourOrderFromX', array('storeName' => getConfig('StoreName'))),
					'L_PAYMENTREQUEST_0_AMT1'		=> number_format($quote->getGrandTotal(),2,'.',''),
		...
		*/
		...
		protected function _ConnectToProvider($transactionURL, $transactionURI, $gateway_postdata)
		{
			...
			if(function_exists("curl_exec")) {
				...
				$result = str_replace('HTTP/1.1 100 Continue ', '', $result);
				$result = explode("\n\r\n", $result);
