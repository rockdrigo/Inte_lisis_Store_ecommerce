/*
 * jQuery UI Nested Sortable 1.0.3
 *
 ...
 *
 * Modified by Ray Ward:
 * - Add a listClass option to apply the specified class to new <ul> elements
 * - Add a serialize function to match the format from the inestedsortable
 * - Add fixClickOnDrag function and bound to sortstop event to resolve a base ui.sortable issue
 *   where click events don't get fired on the first click after sorting in IE and Opera
 */
(function($) {
	...
	$.widget("ui.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		_create: function(){
			...
			this.element.bind('sortstop', this.fixClickOnDrag);
		...
		_mouseDrag: function(event) {
			...
			$(newUl).addClass(this.options.listClass);
		...
		serialize: function()
		{
			var optionElement = this.options.items;
			var idRegExp = this.options.serializeRegExp;

			var buildHash = function(parentList, prefix) {
				var hash = '';

				parentList.children(optionElement).each(function(index) {
					if (hash != '') {
						hash += '&';
					}
					var id = $(this).attr('id').match(idRegExp);
					hash += prefix + '[' + index + '][id]=' + id;

					if ($(this).find('ul').length) {
						var newPrefix = prefix + '[' + index + '][children]';
						hash += '&' + buildHash($(this).find('ul').first(), newPrefix);
					}
			...
			return buildHash(this.element, this.element.attr('id'));
		},

		fixClickOnDrag: function(event, ui)
		{
			// fire a click event on the sorted item. see plugin comment for details.
			ui.item.click();
		}
	}));
