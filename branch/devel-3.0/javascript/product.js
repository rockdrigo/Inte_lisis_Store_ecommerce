...
$(document).ready(function() {
	 ...
	 * Se requiere incluir el Panel "ProductStockDetails.html" en algun lugar de la plantilla product.html y activar el boton de Refrescar Existencias
	 */
	$('#ProductStockDetailRefreshButton').click(function() {
		var mapIndex = 0;
		$('.VariationSelect').each(function() {
			prodVarSelectionMap[mapIndex] = this.value;
			mapIndex++;
		...
		// get the index of this select
		var index = $('.VariationSelect').size();
		...
		if (optionIds == "") {
			optionIds = $('.ProductOptionList input:checked[type=radio] ').val();
		}
		...
		$.getJSON(
			config.AppPath + '/remote.php?w=ProductRefreshStock&productId=' + productId + '&options=' + optionIds,
			function(data) {
				// REQ10046 - Se envia el arreglo obtenido para actializar la tabla
				updateProductDetailStock(data);
			}
		);
	});
	...
	$("Select.ActivatesFieldsByName").change(function() {
		var index = this.selectedIndex;
		var fieldValue = this.options[index].text;
		var fieldId = this.id;
		
		$('.ActivatedByName').hide();
		$('.SelectActivatedByName').each(function() {
			this.value = "";
		});
		
		if(index > 0)
			activateFieldsByName(fieldId, fieldValue);
	});
