	...
	*/
	public function _Get_Smtp_Connection()
	{
		...
		foreach ($responses as $line) {
			...
			if (preg_match('%250[\s|-]auth(.*?)ntlm%i', $line)) {
				$requireauth = true;
				$authtype = 'ntlm';
				break;
			}
		...
		if ($requireauth && $username) {
			...
			if($authtype == 'login') $data = "AUTH LOGIN";
			if($authtype == 'ntlm') $data = "AUTH NTLM";
			...
			if($authtype == 'login'){
				...
				$array_username = explode('\\', $username, 2);
				$user = $array_username[1];
				$domain = $array_username[0];
				$data = base64_encode($this->TypeMsg1($domain, $user));
				...
				$ntlm_res = $this->NTLMResponse(
						substr($response, 24, 8),
						$password
				);
				
				$data = base64_encode($this->TypeMsg3($ntlm_res, $user, $domain, $user));
	...
	{
		$unicode=isc_ASCIIToUnicode($password);
	...
	{
		$domain_unicode=isc_ASCIIToUnicode($domain);
		$domain_length=strlen($domain_unicode);
		$domain_offset=64;
		$user_unicode=isc_ASCIIToUnicode($user);
		$user_length=strlen($user_unicode);
		$user_offset=$domain_offset+$domain_length;
		$workstation_unicode=isc_ASCIIToUnicode($workstation);
