...
function OrderIsComplete($OrderStatus)
{
	if ($OrderStatus == ORDER_STATUS_COMPLETED || $OrderStatus == ORDER_STATUS_SHIPPED) {
		...
		// ORDER_STATUS_AWAITING_FULFILLMENT equals 11, so after that one are user-defined
		if(GetConfig('isIntelisis') && $OrderStatus > ORDER_STATUS_AWAITING_FULFILLMENT){
			$StatusCompletesOrder = $GLOBALS['ISC_CLASS_DB']->FetchOne('SELECT EsCompleto FROM [|PREFIX|]intelisis_order_status WHERE statusid = "'.$OrderStatus.'"', 'EsCompleto');
			if($StatusCompletesOrder && $StatusCompletesOrder == '1'){
				return true;
			}
			else {
				return false;
