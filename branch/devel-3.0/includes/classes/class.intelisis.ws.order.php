...
class ISC_INTELISIS_WS_ORDER extends ISC_INTELISIS_WS {
	...
	public function __construct($orderid) {
		...
		foreach($this->order['products'] as $product)
		{
		  ...
		  */
			...
			if($product['ordprodtype'] == 'giftcertificate'){
				$query_cert_code = 'SELECT gc.giftcertcode
					FROM [|PREFIX|]order_products op
					JOIN [|PREFIX|]gift_certificates gc ON (op.orderorderid=gc.giftcertorderid AND op.total_inc_tax=gc.giftcertamount)
					WHERE op.orderorderid = "'.$product['orderorderid'].'" AND op.orderprodid = "'.$product['orderprodid'].'" AND gc.giftcertcode NOT IN ('.$usedCertCodes.')';
				$cert_code = $GLOBALS['ISC_CLASS_DB']->FetchOne($query_cert_code, 'giftcertcode');
				if($cert_code) {
					$product['ordprodsku'] = 'CR';
					$usedCertCodes .= ', "'.$cert_code.'"';
				}
			}
		...
		if($this->order['ordgiftcertificateamount'] > 0){
			$query_cert_used = 'SELECT gch.histgiftcertid, gch.histbalanceused, gc.giftcertcode
				FROM [|PREFIX|]gift_certificate_history gch
				JOIN [|PREFIX|]gift_certificates gc ON (gch.histgiftcertid=gc.giftcertid)
				WHERE gch.historderid = "'.$this->orderid.'"';
			$result_cert_used = $GLOBALS['ISC_CLASS_DB']->Query($query_cert_used);
			$xml .= '<CertificadosRegalo>';
			while($row_cert_used = $GLOBALS['ISC_CLASS_DB']->Fetch($result_cert_used)){
				$xml .= '<CertificadoRegalo CertID="'.$row_cert_used['histgiftcertid'].'" CodigoCR="'.$row_cert_used['giftcertcode'].'" Monto="'.$row_cert_used['histbalanceused'].'" />';
			}
			$xml .= '</CertificadosRegalo>';
		}
