...
class ISC_INTELISIS_ECOMMERCE_WEBSITUACION extends ISC_INTELISIS_ECOMMERCE {
	...
	private function createUpdateStatus() {
		...
		if($existingId)
		{
			...
			{
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery('intelisis_order_status', array(
						'EsCompleto' => $this->getData('EsCompleto')
						),
						'statusid = "'.$existingId.'"'
				);
		...
		else
		{
			...
			{
				$GLOBALS['ISC_CLASS_DB']->InsertQuery('intelisis_order_status', array(
						'statusid' => $existingId,
						'IDWebSituacion' => $this->getAttribute('IDSituacion'),
						'EsCompleto' => $this->getData('EsCompleto'))
						);
